...
Note that for estimation purposes, under the hood within ```mlsynth```, both the outcome of interest and the auxiliary outcomes are normalized to 100 in the final pre-treatment period. This is to avoid the truly nasty solver issues we would get if we used the raw, untransfomred outcomes that may be on different scales from the target outcome of interest. Note that this is a legal move because this is a linear transformation of the outcome, so it has no effect on the optimality conditions. But it does affect the feasability region. Either way, the user of ```mlsynth``` does not need to manually normalize their outcome metrics.
