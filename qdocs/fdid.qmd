...
Let units be indexed by $i$ and time by $t$. Let $\mathcal{N}$ be the set of all units. Let $i = 0$ denote the treated unit, and $\mathcal{N}_0 \coloneqq \mathcal{N} \setminus {0}$ denotes the control group. Further, denote $\mathcal{U} \subset \mathcal{N}_0$. Parallel trends for Difference-in-Differences (DID) can be formalized as:
