...
After defining which neighborhoods are treated, which serve as controls, and how clusters are formed, the next step is to estimate the treatment effects and quantify uncertainty. The full pre-treatment period is $\mathcal{T}_0 = \{1, \dots, T_0\}$, which in the Curaçao example consists of 128 periods. This period is split into a fitting period $\mathcal{T}_1 = \{1, \dots, T_0 - |\mathcal{B}|\}$, used to compute the synthetic control weights $(\mathbf{w}_j, \mathbf{v}_j)$ and treatment assignments $z_j$, and a blank period $\mathcal{B} = \{T_0 - |\mathcal{B}| + 1, \dots, T_0\}$, withheld for placebo checks. In the example, the blank period has 28 observations. The post-treatment period is $\mathcal{T}_2 = \{T_0 + 1, \dots, T\}$.
