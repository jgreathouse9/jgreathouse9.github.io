...
&= \left\{ \mathbf{w}^{\mathrm{SCM}} \in \mathbb{R}_{\ge 0}^{N_0} \;\middle|\; \mathbf{1}^\top \mathbf{w} = 1 \right\}
\end{aligned}
$$

We seek the weight vector $\mathbf{w}^{\mathrm{SCM}}$ that best matches the treated unit’s pre-treatment outcomes as a weighted average of donor outcomes, subject to the counterfactial predictions being within the span of the donor pool, favoring control units with the highest convex relation to the treated unit.

## SCM for Basque

When we run the optimization, we get that Basque Country is best reproduced by 85% Catalonia and 15% Madrid. The plot below shows how the prediction changes given a change in the unit weights that were selected.
...
T0 = prepped_data["pre_periods"]  # last pre-treatment index
...
def update(frame):
    ...
    # Compute pre-treatment MSE
    mse_pre = np.mean((y[:T0] - y_hat[:T0])**2)
    ...
    if frame == steps - 1:
        ...
        ax.set_title(f"Final SCM: w_Cataluna={w_curr[0]:.2f}, w_Madrid={w_curr[1]:.2f}, "
                     f"Pre-treatment MSE={mse_pre:.2f}")
    else:
...
ani.save("scm_weighted_combo_with_mse.gif", writer=PillowWriter(fps=10))
...
![SCM Weighted Combo](https://raw.githubusercontent.com/jgreathouse9/jgreathouse9.github.io/9c66bf6448807d429b0f940c52ca5e79fc796b36/qprez/scm_weighted_combo.gif)
