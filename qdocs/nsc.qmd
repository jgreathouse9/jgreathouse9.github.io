...
Let $\mathbf{y} \in \mathbb{R}^{T_{0}}$ denote the outcome vector of the treated unit over $T_{0}$ pre-treatment periods. Let $\mathbf{Y}_0 \in \mathbb{R}^{T_{0} \times J}$ denote the matrix of outcomes for $J$ control units over the same pre-treatment periods. We seek to find a vector of weights $\mathbf{w} \in \mathbb{R}^J$ that forms an affine combination of the control units' outcomes to best approximate the treated unit.

We define the vector of pairwise discrepancies $\boldsymbol{\delta} \in \mathbb{R}^J$ as
$$
\delta_j = \|\mathbf{y} - \mathbf{Y}_0\|_2, \quad j = 1, \dots, J,
$$
which captures the $\ell_2$ distance between the treated unit and each control unit in the pre-treatment period. The objective is to solve the following constrained optimization problem:
$$
\begin{aligned}
\min_{\mathbf{w} \in \mathbb{R}^J} \quad & \|\mathbf{y} - \mathbf{Y}_0 \mathbf{w}\|_2^2 + a \sum_{j=1}^J \delta_j |w_j| + b \|\mathbf{w}\|_2^2 \\
\text{s.t.} \quad & \|\mathbf{w}\|_1 = 1
\end{aligned}
$$

The first term, $\|\mathbf{y} - \mathbf{Y}_0 \mathbf{w}\|_2^2$, is a standard least squares loss that penalizes the squared discrepancy between the treated unit's outcomes and the weighted combination of control units. The second term, $a \sum_{j=1}^J \delta_j |w_j|$, introduces a discrepancy-weighted $\ell_1$ penalty on the weights. The scalar parameter $a > 0$ controls the strength of this penalty. This term discourages assigning large weights to control units whose outcomes are far in Euclidean distance from the treated unit. The third term, $b \|\mathbf{w}\|_2^2$, is the $\ell_2$ Tikhonov regularization term that shrinks the weights toward zero in order to stabilize the solution and prevent overfitting. The scalar parameter $b > 0$ determines the degree of shrinkage we do. The constraint $\|\mathbf{w}\|_1 = 1$ is that standard summation constraint. The resulting SC is an affine combination of the control units, maintaining a scale comparability with the treated unit (unlike a linear combination that adopts whatever scale necessary to fit better, for example).

To select appropriate values of the regularization parameters $a$ and $b$, we perform $k$-fold cross-validation over a grid of candidate values. Full transparency: I *had to* go rouge from the original paper. The default CV method that Wei argues for in his paper frankly takes forever (on my end anyways). So, I default to `sklearn`'s cross validation scheme to tune these parameters. This seems to both be quicker and fit about as well as the original approach, but I'm quite happy to take anybody elses suggestions. Anyways, similar to his approach, I treat each control unit in turn as a "pseudo-treated" unit and attempt to reconstruct its pre-treatment outcomes using a weighted combination of the remaining controls. These reconstructions are repeated over several validation folds. In each fold, we partition the control units into a training set $\mathbf{Y}_{\text{train}}$ and a validation set $\mathbf{Y}_{\text{val}}$. For each unit $\mathbf{y}^{(j)}$ in the validation set, we solve the following optimization problem:
$$
\begin{aligned}
...
$$
\text{MSE}^{(j)} = $T_{0}^{-1} \|\mathbf{y}^{(j)} - \widehat{\mathbf{y}}^{(j)}\|_2^2.
$$
...
As we can see, the fit is near perfect: you can barely even see the fit in the pre-intervention period. We also get similar results to Abadie and co (I also tested this with Basque and West Germany and got similar results).
