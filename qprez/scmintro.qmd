...
- The arithmetic mean of a finite sequence of real numbers $(x_1, x_2, \dots, x_n) \in \mathbb{R}^n$ is defined as  
...
- For example, say you have 12 apples and your friend has 18. Denote this as the sequence  $A = (12, 18) \in \mathbb{N}^2$. The mean is
...
- To compute the average, we apply the same formula. Since there are two people, each entry gets weight $1/2$. We multiply each entry by its weight and sum. Thus,

$$
\mathbf{x}^\top \left(\tfrac{1}{2}\mathbf{1}_2\right)
=
\begin{bmatrix}
12 & 18
\end{bmatrix}
...
$$
and the average is computed like
$$
\bar{x} = \mathbf{x} \mathbf{w} 
= \begin{bmatrix} 60000 & 70000 \end{bmatrix} 
\begin{bmatrix} \frac{1}{2} \\[1mm] \frac{1}{2} \end{bmatrix} 
= \frac{1}{2}\cdot 60000 + \frac{1}{2}\cdot 70000 
= 65000 \text{year}.
$$
...
$$
\bar{x} = \mathbf{x} \mathbf{w} 
= \frac{1}{3}\cdot 60000 + \frac{1}{3}\cdot 70000 + \frac{1}{3}\cdot 1.07\times 10^{11}
...
$$
\bar{x} = \mathbf{x} \mathbf{w} 
= 0.5\cdot 60000 + 0.5\cdot 70000 + 0\cdot 1.07\times 10^{11}
= 65000 \,\text{year}.
$$
...
# -----------------------------
url = "https://public.opendatasoft.com/api/explore/v2.1/catalog/datasets/georef-spain-comunidad-autonoma-millesime/exports/geojson/?lang=en&timezone=America%2FNew_York"
geojson_path = "spain_autonomous_communities.geojson"
...
# Keep only acom_code < 18
gdf = gdf[gdf['acom_code'].astype(int) < 18]

# Highlight Basque Country separately
basque = gdf[gdf['acom_code'] == '16']

# -----------------------------
# Step 3: Create Main Map + Inset
# -----------------------------
fig, ax = plt.subplots(figsize=(10, 10))

# Main map: selected communities
gdf.plot(ax=ax, color='#899b8d', edgecolor='white', linewidth=1)
basque.plot(ax=ax, color='#1f77b4', edgecolor='black', linewidth=1.2)

# Title
ax.set_title("Spain", fontsize=16, fontweight='bold', pad=20)
ax.set_axis_off()

# Inset axes for zoomed-in Basque Country
axins = inset_axes(ax, width="35%", height="35%", loc='upper left', borderpad=2)
gdf.plot(ax=axins, color='#f0f0f0', edgecolor='white', linewidth=0.8)
basque.plot(ax=axins, color='#1f77b4', edgecolor='black', linewidth=1.2)

# Zoom inset to Basque Country bounds
xlim = basque.total_bounds[[0, 2]]  # minx, maxx
ylim = basque.total_bounds[[1, 3]]  # miny, maxy
axins.set_xlim(xlim)
axins.set_ylim(ylim)
...
axins.set_xticks([])
axins.set_yticks([])
axins.set_title("Basque Country", fontsize=12, fontweight='bold')
...
- $\mathcal{T}_1 = \{1,\dots,T_0\} \subset \mathbb{N}$: set of **pre-treatment** periods, with cardinality $|\mathcal{T}_1| = T_0$  
- $\mathcal{T}_2 = \{T_0+1,\dots,T\} \subset \mathbb{N}$: set of **post-treatment** periods, with cardinality $|\mathcal{T}_2| = T - T_0$  
- $\mathcal{J} = \{1,\dots,J+1\} \subset \mathbb{N}$: set of all units (1 treated + $J$ controls)  
- $\mathcal{J}_0 = \mathcal{J} \setminus \{1\} = \{2,\dots,J+1\} \subset \mathbb{N}$: set of untreated donor units, with cardinality $|\mathcal{J}_0| = J$
- $\mathbf{y}_1 \in \mathbb{R}^{T}$: outcome vector (e.g., GDP per capita) for the **treated unit** over all $T$ time periods  
- $\mathbf{Y}_0 \in \mathbb{R}^{T \times J}$: donor matrix; each column is a control unit, each row is a time period  
- $\mathbf{w} \in \mathbb{R}^{J}$: vector of donor weights where each entry $w_j$ indicates the contribution of control unit $j \in \mathcal{J}_0$ to the synthetic control
...
- Choose weights to minimize the squared pre-intervention difference between Basque and the weighted donors:  
...
- In practice, this is just OLS on the pre-1975 period, where $\mathbf{y}_1$ is the Basque GDP column and $\mathbf{Y}_0$ contains donor columns.  
- The regression coefficients (betas, $\beta$) are the **weights** $\mathbf{w}^{\ast}$.
- Multiply each control unit by its weight and sum across donors to get predicted Basque GDP pre- and post-intervention.
...
- Define the **period-level treatment effect** as the difference between observed outcome and synthetic control prediction:
...
$$
\text{TTE} = \sum_{t \in \mathcal{T}_2} \hat{\tau}_{1t} 
...
= T_2^{-1} \sum_{t \in \mathcal{T}_2} \Big( y_{1t} - \hat{y}_{1t} \Big)
$$
...
\frac{\frac{1}{T_2} \sum_{t \in \mathcal{T}_2} \hat{\tau}_{1t}}
{\frac{1}{T_2} \sum_{t \in \mathcal{T}_2} \hat{y}_{1t}} \times 100
$$

*Intuition:*  

- $\hat{\tau}_{1t}$ shows the effect at each post-treatment time point.  
- ATT summarizes the overall post-treatment deviation.  
- TTE aggregates deviations into a single cumulative number.  
- PTE scales ATT relative to the counterfactual, giving magnitude in context.
...
### Residuals and Block Permutation Test

- **Pre-treatment residuals:** difference between actual and synthetic control outcomes:
$$
e_t = y_{1t} - \hat{y}_{1t}, \quad t \in \mathcal{T}_1
$$

- **Post-treatment treatment effects:** difference after treatment:
$$
...
$$
N_B = \left\lfloor \frac{T_0}{B} \right\rfloor, \quad
...
- Average absolute post-treatment effect.

---

### Permutation Test

- We now randomly resample pre-treatment blocks with replacement to create synthetic post-treatment residuals $\mathbf{e}^{\ast(m)}$, $m=1,\dots,M$:

$$
S^{\ast(m)} = \frac{1}{T_2} \sum_{t \in \mathcal{T}_2} \big| \mathbf{e}^{\ast(m)}_t \big|
$$

- This generates a null distribution under no treatment.
...
### Confidence Intervals via Pre-Treatment Residuals

- We define the $(1-\alpha)$ quantile of the absolute pre-treatment residuals using the infimum:

$$
q_{1-\alpha} \;=\; 
\inf \Bigg\{ q \in \mathbb{R} \;\Bigg|\; 
\frac{1}{T_0} \sum_{t \in \mathcal{T}_1} 
\mathbf{1} \Bigg( \Big|\, y_{1t} - \sum_{j \in \mathcal{J}_0} w_j \, y_{jt} \,\Big| \le q \Bigg) \ge 1 - \alpha \Bigg\}.
$$
...
$$
\mathrm{CI}_t = 
\Bigg[y_{1t} - \sum_{j \in \mathcal{J}_0} w_jy_{jt} - q_{1-\alpha},y_{1t} - \sum_{j \in \mathcal{J}_0} w_jy_{jt} + q_{1-\alpha} \,\Bigg].
$$
