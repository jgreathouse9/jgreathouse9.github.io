...
The above argument formalizes an intuition we already have in one dimension. If we observe the numbers 6 and 7, we rarely hesitate to say that the number closest to both, under squared distance, is 6.5. Moving slightly to the left of 6.5 increases the distance to 7 more than it decreases the distance to 6; moving slightly to the right does the opposite. At 6.5, these forces exactly balance. The derivative simply encodes this balance condition.


What changes when we move from the numbers $(6,7]$ to the vector $(6,7)$ is not the logic, but the bookkeeping (we will see this later). We are still choosing one scalar, $\mu$, and measuring how far it is from each observation. Writing the data as a vector does not introduce a new problem. From this perspective, the condition

$$
\sum_{i=1}^n (x_i - \mu) = 0
$$

has a simple interpretation: at the optimal value of $\mu$, the upward and downward pulls from the data cancel exactly. Any small change in $\mu$ increases the total squared error, because it moves us away from this point of balance. For me anyways, the calculus derivation really helped me understand this idea better. It reassures us that averaging is not an arbitrary convention. In higher dimensions, where we can no longer "see" this balance by inspection, the same idea will reappear in geometric form as a projection onto a lower-dimensional space.
