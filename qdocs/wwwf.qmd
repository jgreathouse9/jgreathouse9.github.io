...
Now, we can expand the squared Euclidean norm: $f(\beta) = \sum_{t=1}^{T_0} (y_{1t} - \bar{y}_{0t} - \beta)^2.$ To find the value of $\beta$ that minimizes the objective, we take the derivative of $f(\beta)$ with respect to $\beta$. We'll need the [power rule](https://www.youtube.com/watch?v=z1lwai-lIzY) and [chain rule](https://www.youtube.com/watch?v=XIQ-KnsAsbg). We want to compute its derivative with respect to $\beta$:
