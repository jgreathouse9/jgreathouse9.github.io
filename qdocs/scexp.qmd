...
The synthetic control optimization is subject to several constraints that ensure feasibility and reflect practical considerations. The constraints can be written mathematically as:

$$
\begin{aligned}
& w_j \ge 0, \quad v_j \ge 0, \quad \forall j \in [J] \\[1mm]
& \sum_{j \in I_k} w_j = 1, \quad \sum_{j \in I_k} v_j = 1, \quad \forall k \in [K] \\[1mm]
& z_j \in \{0,1\}, \quad w_j \le z_j, \quad v_j \le 1 - z_j \\[1mm]
& \sum_{j \in I_k} z_j = m_{\text{eq},k} \quad \text{(if exact cardinality is specified)},\\
& \sum_{j \in I_k} z_j \ge m_{\text{min},k} \quad \text{(if minimum cardinality is specified)},\\
& \sum_{j \in I_k} z_j \le m_{\text{max},k} \quad \text{(if maximum cardinality is specified)} \\[1mm]
& \sum_{j \in I_k} c_j w_j \le B_k, \quad \forall k \in [K] \quad \text{(if cluster budgets are specified)},\\
& \sum_{k=1}^K \sum_{j \in I_k} c_j w_j \le B_{\text{total}} \quad \text{(if a global budget is imposed)} \\[1mm]
& \sum_{k=1}^K z_{j,k} \le 1, \quad \forall j \in [J] \quad \text{(ensuring each neighborhood is treated in at most one cluster)}
\end{aligned}
$$

The first set of constraints ensures that all treated and control weights are non-negative, which is necessary for forming valid convex combinations of neighborhoods within clusters. The second set normalizes the treated and control weights within each cluster so that they sum to one, guaranteeing that each synthetic treated or control unit represents a proper weighted average of the neighborhoods in the cluster.  

The treatment selection constraints link the binary variable \(z_j\) to the weights, allowing only selected neighborhoods to contribute to the synthetic treated unit, while control weights are zero if a neighborhood is selected. Cardinality constraints can specify the exact number of treated neighborhoods, or set minimum and maximum limits, controlling the size of the treatment group within each cluster.  

Budget constraints limit the total cost of treating neighborhoods, either within each cluster individually or across all clusters collectively. Finally, the exclusivity constraint ensures that each neighborhood is selected for treatment in at most one cluster, preventing overlap and preserving interpretability of the experimental design. Together, these constraints define the feasible set for the synthetic control optimization problem and ensure that the resulting weights are practical, interpretable, and compliant with operational or ethical considerations.
