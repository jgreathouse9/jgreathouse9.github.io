...
Non-negativity and normalization ($w_j, v_j \ge 0$, $\sum w_j = \sum v_j = 1$) mean that synthetic neighborhoods are weighted averages of real neighborhoods. Negative weights would imply "subtracting" a neighborhood’s influence. Summing to one ensures each synthetic neighborhood is in the same scale/mixture of real neighborhoods. Ourv decision variable $z_j \in \{0,1\}, w_j \le z_j, v_j \le 1 - z_j$ asks whether a unit should be treated. Each neighborhood is either treated or not. A treated neighborhood contributes to the synthetic treated unit but not the control unit, and vice versa, avoiding "double-counting."

Practically, analysts may place cardinality constraints $\sum z_j \in [m_{\text{min},k}, m_{\text{max},k}]$ upon the size of the treated group such that at least one neighborhood is treated per cluster to observe effects, but not every neighborhood, respecting ethical, financial, or logistical limits. In a similar vein, budget constraints ($\sum c_j w_j \le B_k$, $\sum c_j w_j \le B_{\text{total}}, especially given that some neighborhoods may have different costs to treat.
...
We form a permutation distribution by pooling the placebo effects $\{\hat{\tau}_{t}^{\mathcal{B}} : t \in \mathcal{B}\}$ and post-treatment effects $\{\hat{\tau}_{t} : t \in \mathcal{T}_1\}$, then repeatedly sampling subsets of size $|\mathcal{T}_1|$ to compute:

$$
S_{\text{perm}}^{(b)} = \frac{1}{|\mathcal{T}_1|} \sum_{t \in \text{sample}} \big|\hat{\tau}_{t}\big|, \quad b = 1, \dots, B.
$$
...
For cluster-level tests, we apply the same procedure using $\{\hat{\tau}_{k t}^{\mathcal{B}} : t \in \mathcal{B}\}$ and $\{\hat{\tau}_{k t} : t \in \mathcal{T}_1\}$ for each cluster $k$, yielding $p_{k,\text{global}}$. These tests assess whether the observed effects are unusually large compared to the null distribution from placebo periods.
