...
$$
\mathbf{u}_{\text{pre}} = \mathbf{y}_{1,\text{pre}} - \mathbf{y}}_{1,\text{pre}}^{\text{SC}}.
$$
Let 
$$
\hat{\sigma}^2 = \frac{1}{T_0 - 1} \left\| \mathbf{u}_{\text{pre}} - \bar{u} \mathbf{1} \right\|^2
$$
be the unbiased estimator of the residual variance, where 
$$
\bar{u} = \frac{1}{T_0} \sum_{t=1}^{T_0} u_t
$$
is the mean residual.

We seek prediction intervals for the counterfactual outcomes in the post-treatment period. Let 
$$
\mathbf{y}}_{1,\text{pre}}^{\text{SC}} \in \mathbb{R}^{T_1}
$$
be the post-treatment synthetic control predictions, where \( T_1 = T - T_0 \). Under the assumption of exchangeability or symmetry in the residuals, we define a symmetric conformal prediction interval around each post-treatment prediction:
$$
\epsilon_\alpha = \sqrt{ \frac{ \hat{\sigma}^2 }{ \alpha / 2 } }.
$$

The lower and upper conformal bounds are then given by
$$
\begin{aligned}
\mathbf{l}_{\text{post}} &= \hat{\mathbf{y}}_{1,\text{post}} - \epsilon_\alpha \mathbf{1}, \\
\mathbf{u}_{\text{post}} &= \hat{\mathbf{y}}_{1,\text{post}} + \epsilon_\alpha \mathbf{1}.
\end{aligned}
$$

This result exploits 
