...
But what does this mean, anyways? What does it mean for tre trend of one time series to be parallel to another one? As it turns out, DID can be written as a simple convex optimization problem where we assume uniform weights over the control units. We define the DID estimator by solving:
...
Okay chain rule over! These are what we cll our first order conditions. Now we can set the derivative equal to zero:
...
Thus, the optimal $\beta^\ast$ is simply the average pre-treatment difference between the treated unit and the control group. What does this mean, then, in terms of parallel pre-intervention trends? This means that for PTA to hold exactly, the difference of the control group, with respect to the treatment group, needs to be equal to $\beta$ at each point in the pre-treatment period (of course in practice we expect this to be a gradient that holds approximately in real life). The degree to which the control group's differs from $\beta$ is where non-parallel pre-intervention trends bias comes from.
