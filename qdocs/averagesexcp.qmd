...
This function is [convex](math.html#def-convex-function) and differentiable. We can optimize this function by taking the derivative with respect to $\mu$. We begin by applying the chain rule term by term. For each term $(x_i - \mu)^2$, there are two parts to consider: the outer function and the inner function. The outer function is $f(u) = u^2$, where $u = x_i - \mu$. Its derivative (by the power rule) is
