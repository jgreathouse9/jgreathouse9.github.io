...
Let $[J] = \{1, \dots, J\}$ denote neighborhoods observed over $T_0$ pre-treatment periods $[T_0] = \{1, \dots, T_0\}$, nested within $K$ clusters $[K] = \{1, \dots, K\}$. Let $I_k \subseteq [J]$ denote the neighborhoods in cluster $k$, with $j \in I_{k(j)}$ indicating cluster membership. Observed outcomes are collected in $\mathbf{Y} \in \mathbb{R}^{J \times T_0}$, where $Y_{j,t}$ represents the outcome for neighborhood $j$ at time $t$. Potential outcomes are $Y_{j,t}^I$ under treatment and $Y_{j,t}^N$ without treatment, with cluster-weighted average treatment effect

$$
\tau_t = \sum_{j=1}^J f_j \big(Y_{j,t}^I - Y_{j,t}^N\big), \quad t > T_0,
$$

where $f_j \ge 0$ and $\sum_{j=1}^J f_j = 1$.

Predictor vectors $\mathbf{x}_j \in \mathbb{R}^r$ (e.g., pre-treatment outcomes or covariates) have cluster mean $\mathbf{\bar{x}}_k = \sum_{j \in I_k} f_j \mathbf{x}_j / \sum_{j \in I_k} f_j$. Distances are defined as $D_{1,j,k} = \|\mathbf{x}_j - \mathbf{\bar{x}}_k\|_2^2$ and $D_{2,j,j',k} = \|\mathbf{x}_j - \mathbf{x}_{j'}\|_2^2$. Optional cardinality constraints $m_{\text{eq},k}, m_{\text{min},k}, m_{\text{max},k}$ specify exact, minimum, or maximum treated units per cluster. Penalization parameters $\beta, \lambda_1, \lambda_2, \xi, \lambda_{1,\text{unit}}, \lambda_{2,\text{unit}} \ge 0$ control specific design choices.
...
We collect all constraints into a feasibility set $\mathcal{F}$:
...
The synthetic control optimization problem is then

$$
\min_{(w,v) \in \mathcal{F}} \mathcal{L}(w,v).
$$

The framework accommodates four main designs:

1. **Base:** Minimizes cluster-level fit between the synthetic treated/control and the cluster mean:

$$
\sum_{k=1}^K \left( \sum_{j \in I_k} f_j \right) \Big(
\|\mathbf{\bar{x}}_k - \sum_{j \in I_k} w_j \mathbf{x}_j\|_2^2
+ \|\mathbf{\bar{x}}_k - \sum_{j \in I_k} v_j \mathbf{x}_j\|_2^2
\Big).
$$

2. **Weak:** Adds similarity between synthetic treated and control predictors:

$$
+ \beta \left\| \sum_{j \in I_k} w_j \mathbf{x}_j - \sum_{j \in I_k} v_j \mathbf{x}_j \right\|_2^2.
$$

3. **Penalized:** Adds distance-based regularization on weights:

$$
+ \lambda_1 \sum_{j \in I_k} w_j D_{1,j,k} + \lambda_2 \sum_{j \in I_k} v_j D_{1,j,k}.
$$

4. **Unit-level:** Adds unit-level penalization:
...
Each design balances fit to the cluster mean and regularization, guiding the selection of treated and control neighborhoods to closely mimic the cluster’s typical profile.
