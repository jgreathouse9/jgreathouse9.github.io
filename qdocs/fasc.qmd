...
$$
\mathcal{W}_{\mathrm{conv}} = \left\{ \mathbf{w} \in \mathbb{R}^{N_0}_{\geq 0} \mid \; \|\mathbf{w}\|_1 = 1 \right\},
$$

$$
\mathcal{W}_{\mathrm{aff}} = \left\{ \mathbf{w} \in \mathbb{R}^{N_0} \mid \|\mathbf{w}\|_1 = 1 \right\},
$$
...
The classic SCM estimator solves the program
...
We seek the weight vector, $\mathbf{w} \in \mathcal{W}_{\mathrm{conv}}$, that minimizes the pre-treatment mean squared error between the treated unit’s observed pre-treatment outcomes $\mathbf{y}_1$ and the weighted average of donor outcomes $\mathbf{Y}_0 \mathbf{w}$.

The forward selection variant constructs a synthetic control by iteratively adding one donor unit at a time to minimize the pre-treatment fit error. Starting from the empty set $\mathcal{S} = \emptyset$ where zero donors are selected, we begin by estimating a single-donor unit SCM. Practically this is akin to finding the neares neighbor, where we estimate $N_0$ submodels. At each iteration, the algorithm examines each donor $j \in \mathcal{N}_0 \setminus \mathcal{S}$ and forms a candidate set $\mathcal{S}^{\prime} = \mathcal{S} \cup \{j\}$. For each such candidate set $\mathcal{S}^{\prime}$, it solves the restricted optimization problem over the donor submatrix $\mathbf{Y}_0^{\mathcal{S}^{\prime}}$, defined as the columns of $\mathbf{Y}_0$ corresponding to units in $\mathcal{S}^{\prime}$:

$$
\mathbf{w}_{\mathcal{S}^{\prime}}^{\ast} = \underset{\mathbf{w} \in \mathcal{W}_{\mathrm{conv}}(\mathcal{S}^{\prime})}{\operatorname*{argmin}} \; \left\| \mathbf{y}_1 - \mathbf{Y}_0^{\mathcal{S}^{\prime}} \mathbf{w} \right\|_2^2,
$$

where

$$
\mathcal{W}_{\mathrm{conv}}(\mathcal{S}^{\prime}) = \left\{ \mathbf{w} \in \mathbb{R}_{\ge 0}^{|\mathcal{S}^{\prime}|} \,\middle|\, \|\mathbf{w}\|_1 = 1 \right\}.
$$

The donor selected at each iteration is

$$
j^{\ast} = \underset{j \in \mathcal{N}_0 \setminus \mathcal{S}}{\operatorname*{argmin}} \; \min_{\mathbf{w} \in \mathcal{W}_{\mathrm{conv}}(\mathcal{S} \cup \{j\})} \left\| \mathbf{y}_1 - \mathbf{Y}_0^{\mathcal{S} \cup \{j\}} \mathbf{w} \right\|_2^2,
$$

and it is then added to the selected set by updating $\mathcal{S} \leftarrow \mathcal{S} \cup {j^\ast}$. 

### Donor Selection and Stopping Rules

The donor selection process proceeds sequentially, with one donor unit added at a time according to the forward selection rule. The procedure may continue until all donor units have been exhausted, but in practice analysts [have several options](https://mlsynth.readthedocs.io/en/latest/fscm.html#fscm-function) for determining when to stop.

In the exhaustive option (the default), selection proceeds until the pre-treatment MSE is minimized over all candidate donor subsets. Let $\mathcal{P}(\mathcal{N}_0)$ denote the power set of the donor indices and define, for any $S \subseteq \mathcal{N}_0$, the within–pre-treatment fit as
$$
\text{MSE}(S) \;=\; \frac{1}{T_0}\,\min_{\mathbf{w}\in\mathcal{W}_{\mathrm{conv}}(S)} \left\| \mathbf{y}_1 - \mathbf{Y}_0^{S}\mathbf{w} \right\|_2^2,
$$
where $\mathcal{W}_{\mathrm{conv}}(S)=\{\mathbf{w}\in\mathbb{R}^{|S|}_{\ge 0}:\|\mathbf{w}\|_1=1\}$. The exhaustive-search estimator chooses
$$
S^\star \;=\; \arg\min_{S\in \mathcal{P}(\mathcal{N}_0)} \text{MSE}(S), 
\qquad 
\mathbf{w}_{S^\star}^\star \;=\; \arg\min_{\mathbf{w}\in\mathcal{W}_{\mathrm{conv}}(S^\star)} \left\| \mathbf{y}_1 - \mathbf{Y}_0^{S^\star}\mathbf{w} \right\|_2^2.
$$

Another option is to rely on a model selection criterion, the modified BIC (mBIC), which provides a trade-off between in-sample fit and model complexity. The modified BIC for a selected set $S$ of donor units is defined as
...
Here, $T_0$ is the number of pre-treatment periods, $\mathbf{y}_1$ is the treated unit’s outcome vector, $\mathbf{Y}_0^S$ is the donor submatrix formed from the selected set $S$, and $\mathbf{w}_S$ is the optimal weight vector estimated on that submatrix. The first term in the criterion rewards better fit, while the second term penalizes larger donor sets. As in standard information criteria, smaller values of mBIC are preferred, and the forward selection procedure may be terminated as soon as adding a new donor increases the penalized error:

$$
\text{mBIC}(S \cup \{j\}) > \text{mBIC}(S).
$$

A final option is to impose an upper bound on the number of selected donors. When a cardinality cap is imposed, with $p\in(0,1]$ and $K=\lfloor pN_0\rfloor$, the search is restricted to $\{S\subseteq \mathcal{N}_0:\, |S|\le K\}$, so that
$$
S^\star \;=\; \arg\min_{\substack{S\subseteq \mathcal{N}_0\\ |S|\le K}} \text{MSE}(S),
\qquad 
\mathbf{w}_{S^\star}^\star \;=\; \arg\min_{\mathbf{w}\in\mathcal{W}_{\mathrm{conv}}(S^\star)} \left\| \mathbf{y}_1 - \mathbf{Y}_0^{S^\star}\mathbf{w} \right\|_2^2.
$$

I programmed this for flexibility in practice: in low-dimensional settings analysts may choose to evaluate all donors with minimal runtime, whereas in high-dimensional applications it is neither computationally efficient nor substantively plausible to evaluate extremely large subsets, and early stopping or capped selection provides a practical alternative. The algorithm issues a warning if the user has a donor pool of at least 200.
...
The Forward-Selected Augmented Synthetic Control (FASC) estimator synthesizes two complementary strategies in SCM research: forward selection of donors and bias correction via the discrepancy penalty, $\left\| \mathbf{w} - \mathbf{w}_0 \right\|_2^2$. It begins with a forward-selected SCM (FSCM) as described above, producing a base weight vector $\mathbf{w}_0$. In this procedure, let $\mathcal{S}_0 \subseteq \mathcal{N}_0$ denote the set of donor indices selected by the FSCM corresponding to the non-zero entries of $\mathbf{w}_0$. The full donor weight vector is then defined as

$$
w_j^{\mathrm{full}} = w_{0,j} \, \mathbf{1}_{\{j \in \mathcal{S}_0\}}, \quad j \in \mathcal{N}_0,
$$

where $\mathbf{1}_{\{j \in \mathcal{S}_0\}}$ is an indicator function equal to $1$ if $j \in \mathcal{S}_0$ and $0$ otherwise. This ensures that only the selected donor indices carry the forward-selected weights, while all other donors are assigned zero weight. FASC then augments this base model by allowing an affine combination of the donors to potentially improve the pre-treatment fit, using $\mathbf{w}^{\mathrm{full}}$ as a warm start. The FASC estimator solves

$$
\mathbf{w}^\ast_{\mathrm{FASC}} = \underset{\mathbf{w} \in \mathcal{W}_{\mathrm{aff}}}{\operatorname*{argmin}} \; \left\| \mathbf{y}_1 - \mathbf{Y}_0 \mathbf{w} \right\|_2^2 + \lambda \left\| \mathbf{w} - \mathbf{w}^{\mathrm{full}} \right\|_2^2,
$$

where $\lambda \ge 0$ controls the strength of the bias-correction toward the forward-selected weights.

The solution lies in the affine hull of all donors, but it is regularized to remain close to the sparse, interpretable model obtained from forward selection on the condition that the originally selected model is good. The strength of this regularization is governed by the parameter $\lambda$. The regularization parameter $\lambda$ is selected via time-split cross-validation on the pre-treatment period. First, use the first half ($t = 1, \dots, \lfloor T_0/2 \rfloor$) to fit candidate weights $\mathbf{w}(\lambda)$. Then, use the second half ($t = \lfloor T_0/2 \rfloor + 1, \dots, T_0$) to evaluate prediction error. For each candidate $\lambda$, we compute:
...
Practitioners can apply FASC to datasets with high-dimensional donor pools without manually specifying which donors to include, saving time and reducing bias from subjective decisions. By prioritizing donors that maximize pre-treatment balance, FASC mitigates overfitting, while simultaneously mitigating under-fitting via the discrepancy penalty.

For all the talk around Geolift's market selection mechanism, it uses dynamic time warping under the hood to select control units. While not wrong, this method may be slower than forward selection because forward selection operates on a smaller feature space (covariates rather than entire time series) and avoids pairwise time series alignments. In contrast, DTW (naively at least) may not be well suited for high dimensional settings such as settings where we have many more control units than pre-treatment periods. The other option for GeoLift is the correlation option, but simple correlation ranking may not be ideal for more complex time series data.

As usual, more can be done here. In many real-world settings, treatments are not applied simultaneously but occur at different times across units (staggered adoption). Traditional synthetic control methods assume a single treatment point, which can limit their applicability. In principle, one could adopt [a pooled setting](https://doi.org/10.1111/rssb.12448) where we trade off between fitting synthetic controls as individual units versus fitting to the treated unit's as an average. However, this would require derivation of Cohort ATTs to deal with the setting where all treated units do not share the same calendar time of treatment. 
