...
The forward selection procedure starts by choosing a single donor unit. Each donor $j \in \mathcal{N}_0$ is evaluated independently by solving:

$$
\underset{\mathbf{w} \in \Delta^0(\{j\})}{\operatorname*{argmin}} \, \| \mathbf{y}_{1} - \mathbf{Y}_{0}\mathbf{w} \|_2^2 \forall t \in \mathcal{T}_1.
$$

This computes one MSE per donor unit (essentially identifying the nearest neighbor in terms of pre-treatment fit). The donor with the lowest MSE, denoted as $j^\ast$, is selected as the first element of the donor set:

$$
 j^\ast = \underset{j \in \mathcal{N}_0}{\operatorname*{argmin}} \, \| \mathbf{y}_{1} - \mathbf{Y}_{0} \mathbf{w}^{(j)} \|_2^2 \forall t \in \mathcal{T}_1.
$$

We define this initial set as $\mathcal{S}_1 = \{j^\ast\}$. To generalize, let $\mathcal{S}_1, \mathcal{S}_2, \dots, \mathcal{S}_K$ denote the donor sets selected at each iteration, with corresponding MSE values $MSE(\mathcal{S}_1), MSE(\mathcal{S}_2), \dots, MSE(\mathcal{S}_K)$. At each subsequent iteration $K$, we expand the donor pool by adding a new unit $j^\ast$ to the previous set $\mathcal{S}_{K-1}$. Specifically, for each candidate donor $j \in \mathcal{N}_0 \setminus \mathcal{S}_{K-1}$, we update the synthetic control model and select the unit that minimizes the MSE over the expanded subsimplex:
...
The updated donor set is then defined as:

$$
\mathcal{S}_K = \mathcal{S}_{K-1} \cup \{j^\ast\}
$$
With each subsequent set having its own MSE. Our final donor pool is selected as
$$
\mathcal{S}^\ast = \underset{\mathcal{S}_i \in \{\mathcal{S}_1, \mathcal{S}_2, \dots, \mathcal{S}_K\}}{\operatorname*{argmin}} \, MSE(\mathcal{S}_i),
$$
or the set of donors that minimizes the MSE. Note of course that not all donors will receive positive weight here.
