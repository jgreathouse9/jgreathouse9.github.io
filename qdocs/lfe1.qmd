...
To begin, consider the fact that the SCM is a form of inner products between a vector and a matrix. So, as a result, we may rely on the [Hilbert Projection Theorem](https://www.youtube.com/watch?v=eRCkfBmnzqk).
...
Now we just apply these rules to $f(w_1) = (2 + 28 w_1)^2 + (10 + 28 w_1)^2$. Since the function is a sum of two terms, we use the sum rule to differentiate each term separately.


Here is the inner function of term one:

$$
u = 2 + 28 w_1.
$$

and here is the outer function:
...
$$
g(u) = u^2 \implies g'(u) = 2u.
$$

Now let's do the inner function:
$$
h(w_1) = 2 + 28 w_1 \implies h'(w_1) = 28.
$$

By application of the chain rule,
$$
\frac{d}{dw_1} (2 + 28 w_1)^2 = g'(h(w_1)) \cdot h'(w_1) = 2 (2 + 28 w_1) \cdot 28.
$$

Now let's do the same for term two. We have the inner function:
...
We apply the power rule to the outer function:
$$
g(v) = v^2 \implies g'(v) = 2v.
$$

Now we differentiate the inner function:
$$
h(w_1) = 10 + 28 w_1 \implies h'(w_1) = 28.
$$

Now by the chain rule,
$$
\frac{d}{dw_1} (10 + 28 w_1)^2 = g'(h(w_1)) \cdot h'(w_1) = 2 (10 + 28 w_1) \cdot 28.
$$

Okay these are the first two terms. Using the sum rule, the derivative of $f(w_1)$ is the sum of the derivatives of the two terms:

$$
f'(w_1) = \frac{d}{dw_1} (2 + 28 w_1)^2 + \frac{d}{dw_1} (10 + 28 w_1)^2 = 2 (2 + 28 w_1) \cdot 28 + 2 (10 + 28 w_1) \cdot 28.
$$

Factor out the common terms:

$$
f'(w_1) = 2 \cdot 28 \cdot (2 + 28 w_1) + 2 \cdot 28 \cdot (10 + 28 w_1) = 56 (2 + 28 w_1) + 56 (10 + 28 w_1).
$$

Combine the terms inside:

$$
(2 + 28 w_1) + (10 + 28 w_1) = 2 + 10 + 28 w_1 + 28 w_1 = 12 + 56 w_1.
$$
...
To find critical points of the objective function

$$
f(w_1) = \|\mathbf{y} - \hat{\mathbf{y}}(w_1)\|^2,
$$

we first set the derivative to zero:

$$
f'(w_1) = 56(12 + 56 w_1) = 0 
\implies w_1 = -\frac{3}{14} \approx -0.214.
$$

This is the unconstrained minimum. However, this value of $w_1$ lies outside the feasible set $[0,1]$ of convex weights. Since this is outside the feasible set $[0,1]$, the Hilbert Projection Theorem guarantees the true minimizer lies on the boundary of the convex hull.

Evaluate the objective function at the boundaries:
...
As we can see, it is a copy of one of the donors. To confirm this, we can look at the residual orthogonality condition


::: {.lemma #residual-orthogonality}
**Residual Orthogonality Condition.** Let $\hat{\mathbf{y}}$ be the projection of $\mathbf{y}$ onto a convex set $C$. Then for all $\mathbf{z} \in C$:
$$
\langle \mathbf{y} - \hat{\mathbf{y}}, \mathbf{z} - \hat{\mathbf{y}} \rangle \le 0.
$$
:::

The residual vector is just the difference between the target vector and the synthetic control:

$$
\mathbf{r} = \mathbf{y} - \hat{\mathbf{y}} = \begin{bmatrix} 2 \\ 10 \end{bmatrix}.
$$

The direction spanning $C$ from $\hat{\mathbf{y}} = \mathbf{y}_2$ is:

$$
\mathbf{y}_1 - \mathbf{y}_2 = \begin{bmatrix} -28 \\ -28 \end{bmatrix}.
$$

Check the Hilbert Projection condition:

$$
\langle \mathbf{r}, \mathbf{y}_1 - \mathbf{y}_2 \rangle = 2(-28) + 10(-28) = -336 \le 0.
$$

The negative dot product confirms that moving toward $\mathbf{y}_1$ would increase the distance, so $\hat{\mathbf{y}} = \mathbf{y}_2$ is indeed the projection onto the closest face of the convex hull, exactly as predicted by the Hilbert Projection Theorem.

We further know this must be true because of [Minkowski-Weyl theorem](https://www.youtube.com/watch?v=RquUIXUMLcc)
...
If we want, we can even check whether we have arrived at the minimal point. To do this, we need to derive the [Karush-Kuhn-Tucker](https://mdav.ece.gatech.edu/ece-3803-fall2021/notes/17-notes-3803-f21.pdf)
...
Now we confirm the KKT conditions for the optimization problem:
