...
- **Connection to Difference-in-Differences (DiD)**  
  - DiD assumes outcomes follow:  
    ...
    $$
    where $\mu_j$ are unobservable unit factors and $\lambda_t$ are time variant, unit constant factors (note they are additive) 
...
- For example, say you have 12 apples and your friend has 18. Denote this as set $A = \{12, 18\} \subset \mathbb{N}$:

$$
15 = \bar{x} = \frac{1}{2} \sum_{x \in A} x
...
- Here, set $A$ is mapped on to a streadsheet, where each column entry represents a person. The value in each column is the number of apples that person has.
...
- This is mathematically equivalent to dividing the sum of the entries by 2 that we did from the scalar tab. In other words, arithmetic averaging is just taking a weighted sum where each weight is the reciprocal of the number of entries.
...
- Now, we have columns *and* rows. Each row represents a snapshot in time. Each column represents a person.
...
- Row-wise averaging is thus just taking a weighted sum across the columns for each row. Each row "collapses" into a single number representing the average for that time period.  
...
- Well, why do we not try this? After all, there is no law of nature that says the weight needs to be the reciprocal of the number of observations. Practically, we can assign weights however we like.

- Instead of treating Bill as equal to you two, we can give each person a weight $w_i \in [0,1]$ to decide how much they count in the average. This weight $w_i$ is equivalent to the fraction we used before in the arithmetic average, which previously was simply $\tfrac{1}{n}$. Our weights may take the form:  
...
Now the average is back to what it was initially. Bill clearly is not representative of a bar or even a country. Giving him as much weight as regular humans does not make sense.
...
- In the earlier examples, the weights we used were arbitrary. We split 50/50 between you and your friend, but there was no real reason for that choice. In real life anyways, weights are never given to us from the Heavens: we, the econometricians, must assign them. The trouble is that assigning them poorly can create nonsense averages.  

- The arithmetic average is familiar to economists as the national. It is used all the time in policy discussions from [militarized crackdowns on crime]((https://www.pbs.org/newshour/politics/trump-deploys-national-guard-to-memphis-calling-it-a-replica-of-his-crackdown-on-washington)), [healthcare spending](https://www.cnn.com/2025/03/07/politics/medicaid-cuts-republican-districts-analysis), and [gas prices](https://calmatters.org/commentary/2024/10/political-theater-californias-gasoline-prices/).

- But again, this still presumes the aggregate weights provide a more meaningful comparison for a single unit than they actually do.

### Basque

- Suppose we have one unit exposed to terrorism in 1975 (the Basque Country), and a set of 16 other units that are not exposed. We want to estimate the effect of terrorism on GDP per capita via taking a weighted average of control units, but how to choose the weights? The Basque Country is on average much wealthier than many areas of Spain, has a unique political climate, and was more educated and industrialized than other areas.

- Optimization solves this problem. Instead of us just arbitrarily assigning weights, we *choose the weights that minimize the pre-intervention gap* between the Basque Country and the weighted donor units.  
...
One method of optimization we could use is ordinary least-squares. In multiple linear regression, the coefficients returned to us can be interpreted as weights for our control group.
...
### Block Permutation Inference {#block-permutation}

Block permutation inference assesses the statistical significance of synthetic control treatment effects while accounting for temporal dependence in pre-treatment residuals.

#### Notation

- Treated outcomes: $\mathbf{y}_1 \in \mathbb{R}^{T}$  
- Synthetic control outcomes: $\hat{\mathbf{y}}_1 \in \mathbb{R}^{T}$  
- Pre-treatment period: $\mathcal{T}_1 = \{1, \dots, T_0\}$  
- Post-treatment period: $\mathcal{T}_2 = \{T_0+1, \dots, T\}$  
...
Block size $B$ is chosen as the smallest lag such that $|\text{ACF}(B)| < \text{acf\_threshold}$.
...
#### Step 3: Partition residuals into blocks
...
#### Step 4: Compute observed global statistic
...
#### Step 6: Split-conformal confidence intervals
...
For $t \in \mathcal{T}_1$, set $\text{CI}_t = \text{NaN}$.
...
def block_permutation_inference(Y_treated, Y_control, T0, alpha=0.05, max_combinations=1000, acf_threshold=0.1,
                                random_state=None):
    """
...
# --- Usage example ---
plot_scm_treatment_effect(obs, cf, T0, CI=inference["CI"])
