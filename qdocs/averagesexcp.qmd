...
The above argument formalizes an intuition we already have in one dimension. If we observe the numbers 6 and 7, we rarely hesitate to say that the number closest to both, under squared distance, is 6.5. Moving slightly to the left of 6.5 increases the distance to 7 more than it decreases the distance to 6; moving slightly to the right does the opposite. At 6.5, these forces exactly balance. The derivative simply encodes this balance condition. For me anyways, the calculus derivation really helped me understand this idea better.
...
While I understand the intent behind this phrasing, I believe it is, or at the very least has a high potential to be, misleading. When we use works like "fake" or "Frankenstein", these are loaded terms that suggest deception or haphazardly grafting together data points. However as we have just seen, the arithmetic mean does exactly this. The national mean income over time? Takes all incomes across individuals and time points, assigns each person a weight, and computes the weighted sum. If we're in online retail and we care about the mean basket size, we're taking the size of each basket and computing a sum where each basket is given the same weight. Yet we do not describe the national average income as "Franken-Income," nor the weekly mean of basket size as a "Franken-Basket."


Likewise, the idea of a "[fake](https://discord.com/blog/measuring-product-impact-without-a-b-testing-how-discord-used-the-synthetic-control-method-for-voice-messages) control group" obscures the fact that every unit in the donor pool is a real, live observed control unit that was unexposed to the treatment. Synthetic control predictions, like any other kind of average, do not fabricate data at all. They simply summarize data by weighting observations per some loss function. The arithmetic mean *feels* objective only because the optimization problem that defines it has been hidden by decades of pedagogy. In other words, the SCM weights may provoke uncertainty in users because the weights may vary, and this variance suggests some kind of Decision at work. In contrast, the arithmetic mean weights are trivial, so they are implicitly framed as Not a Decision, just a pure computational fact.

These descriptions become even more puzzling, however, once we notice that the arithmetic mean already satisfies the classic SCM constraints. Recall the formula

$$
\bar{y} = \frac{1}{n} \sum_{i=1}^n y_i.
$$

for the average of some outcome variable. We can rewrite this as a weighted sum by setting $w_i = 1/n \: \forall \, i$, yielding $\sum_{i=1}^n y_i w_i$. Given that $n$ is a positive integer, these weights are non-negative sum to one. This precisely the defining constraint set of the original SCM. We do not talk about it like it is, but that's the case. The arithmetic mean of the donor pool, therefore, is a special case of SCM. If a client asked me to compute the average of sales across markets over time and I replied, "You mean you want me to make a Frankenstein monster of your sales data?" they would rightly think I had lost the plot. Presumably this is also why the uniform weights (as in, say, [Difference-in-Differences](https://arxiv.org/abs/1610.07748)) are never questioned, but the weights from SCM are met with suspicion. When the weights are framed openly as a decision, it becomes a thing we may attack or defend, I guess.


The point of this post is simple: every single estimator in this book uses a weighted average of controls which is derived from an optimization problem, whether we are discussing Difference-in-Differences or SCM. It is the task of the econometrician or data scientist to know when which flavor of weights are called for and why. In the remainder of the chapter, we generalize this idea from scalars to vectors and matrices. What matters is not whether we average, but how the weights are chosen.
