...
When we run the regression model, we get that Basque Country is best reproduced by 85% Catalonia and 15% Madrid. The plot below shows how the prediction itself, as well as the pre-treatment Root Mean Squared Error changes, given a change in the unit weights that were selected.
...
line_y, = ax.plot(time, y, label="Treated (Basque)", color="red", linewidth=2)
line_hatY, = ax.plot(time, w_start[0]*Y0[:,0] + w_start[1]*Y0[:,1], 
                     label="Weighted combo", color="purple", linestyle="--")
line_cataluna, = ax.plot(time, Y0[:,0], label="Cataluna", color="green", alpha=0.5)
...
ax.axvline(x=time[T0], color="black", linestyle="--", alpha=0.8, label="Terrorism")
...
def update(frame):
    ...
    if frame == steps - 1:
        ...
        ax.set_title(
            f"Frame {frame+1}: w_Cataluna={w_curr[0]:.2f}, w_Madrid={w_curr[1]:.2f}, "
            f"Pre-treatment RMSE={rmse_pre:.3f}"
        )
