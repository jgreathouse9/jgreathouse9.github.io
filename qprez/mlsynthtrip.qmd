...
- Provide diagnostics or effect-size statistics automatically, and as a result
- facilitate comparison between methods.  

**Impact:** These barriers slow down analysis, create decision-making bottlenecks, and increase inefficiency.

---

## Practical Solutions

A streamlined workflow should include:  

- **Input:** a long table (one row per unit per time) with columns for:  
  - Unit name  
  - Time  
  - Treatment indicator (1 = treated, 0 = control)  
  - Outcome  
- **Method:** a single `.fit()` call that handles all computations  
