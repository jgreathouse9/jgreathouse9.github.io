...
The "weak" design adds a penalty to encourage similarity between the synthetic treated and synthetic control units. In practice, this design tends to select treated units that are more representative of the actual treated population rather than the entire cluster. That is, it approximates the Average Treatment Effect on the Treated (ATT) rather than the full Average Treatment Effect (ATE). This is useful if you are targeting a single or very small set of units of interest at first.
...
The "penalized" design introduces extra penalties based on distances from cluster means. Overfitting is a concern in SCM settings where we have very many donors to choose from, and the solution may be non-unique. Adding the penalties helps ensure that the chosen treated and control units are as close as possible to the cluster average, which is especially helpful when some neighborhoods are extreme outliers or when you want to enforce stricter fidelity to the cluster profile.
...
# Constraints in Clustered Synthetic Control Optimization

In causal inference, clustered synthetic control methods help estimate treatment effects without randomization by constructing synthetic treated and control groups within clusters of similar units, such as neighborhoods or markets. The optimization problem assigns weights $w_j$ (for synthetic treated units) and $v_j$ (for synthetic control units) to units $j \in \mathcal{J} = \{1, \dots, J\}$, nested within clusters $k \in \mathcal{K} = \{1, \dots, K\}$, where $I_k \subseteq \mathcal{J}$ denotes units in cluster $k$. A binary variable $z_j \in \{0,1\}$ indicates whether unit $j$ is treated (1) or not (0). The constraints, which define the feasible set $\mathcal{F}$, ensure the resulting design is practical, interpretable, and aligned with real-world limitations like budgets or experimental needs. Below, each constraint is explained, blending its mathematical role with its practical significance.

## Constraints

The optimization is subject to the following constraints:

$w_j \ge 0, \quad v_j \ge 0, \quad \forall j \in [J]$  
$\sum_{j \in I_k} w_j = 1, \quad \sum_{j \in I_k} v_j = 1, \quad \forall k \in [K]$  
$z_j \in \{0,1\}, \quad w_j \le z_j, \quad v_j \le 1 - z_j$  
$\sum_{j \in I_k} z_j = m_{\text{eq},k} \quad,$  
$\sum_{j \in I_k} z_j \ge m_{\text{min},k},$  
$\sum_{j \in I_k} z_j \le m_{\text{max},k} \quad \text{(if maximum cardinality is specified)}$  
$\sum_{j \in I_k} c_j w_j \le B_k, \quad \forall k \in [K] \quad,$  
$\sum_{k=1}^K \sum_{j \in I_k} c_j w_j \le B_{\text{total}}$  
$\sum_{k=1}^K z_{j,k} \le 1, \quad \forall j \in [J]$.

Non-Negativity is the one we're all familiar with, $w_j \ge 0, \quad v_j \ge 0, \quad \forall j \in [J]$. This constraint ensures that weights $w_j$ (for the synthetic treated unit) and $v_j$ (for the synthetic control unit) for each unit $j$ are never negative. In synthetic control methods, synthetic units are built as weighted averages of real units, like neighborhoods or stores, to mimic the treated unit’s behavior (e.g., sales or visitor trends). Allowing negative weights would mean subtracting a unit’s characteristics, which doesn’t make sense—like saying a neighborhood contributes “negative customers” to a synthetic market. By enforcing $w_j \ge 0$ and $v_j \ge 0$, the synthetic units stay within the convex hull of observed data. Hand in hand with this is the summation constraint, or $\sum_{j \in I_k} w_j = 1, \quad \sum_{j \in I_k} v_j = 1, \quad \forall k \in [K]$. Within each cluster $k$, the weights $w_j$ for the synthetic treated unit and $v_j$ for the synthetic control unit must sum to 1 across all units $j \in I_k$. This ensures that each synthetic unit is a proper weighted average, acting like a proportion of the real units’ characteristics, such as pre-treatment sales or foot traffic. For example, in a cluster with three neighborhoods, the synthetic treated unit might combine 50% of one neighborhood’s data, 30% of another’s, and 20% of the third’s, with $w_1 + w_2 + w_3 = 1$. The same applies to the control unit’s weights $v_j$. This normalization keeps the synthetic units directly comparable to the real treated unit, avoiding arbitrary scaling that could distort the causal effect estimate. It’s a standard feature of synthetic controls, ensuring the synthetic unit reflects a balanced combination of real data.

The first major constraint that differs from the standard setup is the treatment selection constraint, or mutual exclusivity: $z_j \in \{0,1\}, \quad w_j \le z_j, \quad v_j \le 1 - z_j$. These constraints tie the binary treatment indicator $z_j$ (1 if unit $j$ is treated, 0 otherwise) to the weights, ensuring a clear separation between treated and control groups. The condition $z_j \in \{0,1\}$ means each unit is either treated or not—no partial treatment assignments. The constraints $w_j \le z_j$ and $v_j \le 1 - z_j$ ensure that a treated unit ($z_j = 1$) can contribute to the synthetic treated unit ($w_j \le 1$) but not the control unit ($v_j = 0$), while a non-treated unit ($z_j = 0$) can contribute to the control unit ($v_j \le 1$) but not the treated unit ($w_j = 0$). This prevents a unit, like a store running a marketing campaign, from being used in both the treated and control groups, which would muddy the counterfactual and bias the treatment effect estimate. It’s critical for maintaining a clean experimental design.

Analysts may place cardinality  constraints, expressed as: $\sum_{j \in I_k} z_j = m_{\text{eq},k}$, $\sum_{j \in I_k} z_j \ge m_{\text{min},k}$, $\sum_{j \in I_k} z_j \le m_{\text{max},k}$. These constraints control how many units in each cluster $k$ are treated, based on the binary indicator $z_j$. The sum $\sum_{j \in I_k} z_j$ counts treated units in the cluster. If an exact number is needed, $m_{\text{eq},k}$ specifies it (e.g., exactly two units treated). Alternatively, $m_{\text{min},k}$ sets a minimum (e.g., at least one unit) and $m_{\text{max},k}$ sets a maximum (e.g., no more than three units). This flexibility lets experimenters balance statistical power—needing enough treated units for reliable estimates—with practical limits, like avoiding treating too many units due to cost or ethical concerns. For instance, a business might limit the number of stores running a campaign in a cluster to manage expenses while ensuring enough data for analysis. These constraints make the design adaptable to real-world needs.

We also have budget constraints, or $\sum_{j \in I_k} c_j w_j \le B_k, \quad \forall k \in [K]$, $\sum_{k=1}^K \sum_{j \in I_k} c_j w_j \le B_{\text{total}}$. These linear constraints cap the cost of treating units. Each unit $j$ has a cost $c_j \ge 0$ (e.g., the expense of running a campaign in a neighborhood). The cluster-level constraint $\sum_{j \in I_k} c_j w_j \le B_k$ ensures that the weighted cost of treated units in cluster $k$ stays within the cluster’s budget $B_k$. The global constraint $\sum_{k=1}^K \sum_{j \in I_k} c_j w_j \le B_{\text{total}}$ limits the total cost across all clusters. Since $w_j$ can be fractional, this allows partial treatment (e.g., a scaled-down campaign in a unit), giving flexibility to optimize within budget. For example, a company might allocate $10,000 per cluster and $50,000 overall for a marketing test, ensuring the experiment is financially viable while still constructing a synthetic treated unit. This directly addresses operational constraints, like minimizing resource use.

The exclusivity constraint, or $\sum_{k=1}^K z_{j,k} \le 1, \quad \forall j \in [J]$, ensures that each unit $j$ is treated in at most one cluster, as $\sum_{k=1}^K z_{j,k} \le 1$ limits the treatment indicator across all clusters. If a unit, like a store, is treated in one cluster (e.g., with a specific campaign), it can’t be treated in another cluster with a different setup. This prevents overlap that could confuse the treatment effect, as a unit exposed to multiple treatments would have mixed outcomes, undermining causal clarity. For example, a neighborhood can’t run two different marketing campaigns in separate clusters, as this would make it hard to isolate which campaign drove results. This constraint keeps the experimental design clean and interpretable, ensuring each unit’s role (treated or control) is unambiguous.
