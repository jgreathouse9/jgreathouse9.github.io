...
The paper studies the pass through rate of abolishing the tax on savings to consumers.
...
def plot_treated_vs_controls(donor_matrix, treated_vector, pre_periods, title):
    ...
    plt.plot(donor_matrix, color='gray', linewidth=0.5, alpha=0.8, label='_nolegend_')
...
df = dataprep(df, 'ITEM_ID', 'INDEX_DATE, ITEM_INDEX, 'Tax')

plot_treated_vs_controls(prepped["donor_matrix"], prepped["y"], prepped["pre_periods"], "Treated Unit vs. Controls")
...
Here is a re-analysis of this question using ```mlsynth```. I use the $\ell_2$ relaxer and Forward DID (FDID).
