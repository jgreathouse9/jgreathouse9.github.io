...
We seek the weight vector $\mathbf{w} \in \mathcal{W}_{\mathrm{conv}}$ that minimizes the mean squared error between the treated unit’s observed pre-treatment outcomes $\mathbf{y}_1$ and the weighted average of donor outcomes $\mathbf{Y}_0 \mathbf{w}$ over the pre-treatment period $\mathcal{T}_1$. The forward selection variant constructs a sparse synthetic control by iteratively adding one donor unit at a time to minimize the pre-treatment fit error. Starting from the empty set $\mathcal{S} = \emptyset$ of selected donors, at each iteration, the algorithm examines each donor $j \in \mathcal{N}_0 \setminus \mathcal{S}$ and forms a candidate set $\mathcal{S}^{\prime} = \mathcal{S} \cup \{j\}$. For each such candidate set $\mathcal{S}^{\prime}$, it solves the restricted optimization problem over the donor submatrix $\mathbf{Y}_0^{\mathcal{S}^{\prime}}$, defined as the columns of $\mathbf{Y}_0$ corresponding to units in $\mathcal{S}^{\prime}$:
...
and it is then added to the selected set by updating $\mathcal{S} \leftarrow \mathcal{S} \cup {j^\ast}$. This process repeats until a stopping rule is met, such as a maximum number of donors or minimal improvement in fit. The donor selection process continues [at the discretion of the user](https://mlsynth.readthedocs.io/en/latest/fscm.html#fscm-function), until either all/some portion of donors have been exhausted, or a model selection criterion, the modified BIC, begins to increase and `full_selection = FALSE`. The modified BIC is defined as:
