...
Unlike randomized controlled trials (RCTs), which rely on random assignment to balance treatment and control groups in both levels and trends, DID is designed for observational data where such balance rarely holds. DID acknowledges that baseline differences may exist, and instead assumes that the treated and control groups would have followed parallel trends in the absence of treatment. The parallel trends assumption states that the expected difference between the treated unit's counterfactual outcome $y_{1t}(0)$ and the control group’s mean outcome $\bar{y}_{0t}$ remains constant over time:

$$
\mathbb{E}[y_{1t}(0) - \bar{y}_{0t} \mid t \in \mathcal{T}_2] = \mathbb{E}[y_{1t}(0) - \bar{y}_{0t} \mid t \in \mathcal{T}_1]
$$

Equivalently, we can restate this in terms of time differences. For any $t, s \in \mathcal{T}_2$:

$$
\mathbb{E}[y_{1t}(0) - y_{1s}(0)] = \mathbb{E}[\bar{y}_{0t} - \bar{y}_{0s}]
$$

Since $y_{1t}(0)$ is observed in the pre-treatment period (that is, $y_{1t}(0) = y_{1t}$ for $t \in \mathcal{T}_1$), we can use this to check the plausibility of parallel trends:

$$
\mathbb{E}[y_{1t} - y_{1s}] = \mathbb{E}[\bar{y}_{0t} - \bar{y}_{0s}] \quad \text{for all } t, s \in \mathcal{T}_1
$$

This allows us to focus on differences in trends rather than levels. So even if the treated and control groups differ in their baseline outcomes... DID remains valid as long as the pre-treatment trends are sufficiently parallel.

DID can be written as a simple convex optimization problem where we assume uniform weights over the control units. We define the DID estimator by solving:
...
Here, the weights $w_j$ are fixed to be uniform. That is, DID does not even estimate the weights; it takes the arithmetic average of the control outcomes and simply adjusts for the mean level difference between that average and the treated unit. In other words, they are imposed instead of analytically derived. The only parameter to solve for is $\beta$, the scalar that minimizes the pre-treatment discrepancy.

But how can we tell? Let us behold the closed form solution. I'll confess to never having done this before (or at least not since I solved least squares by hand!!!), so this was actually quite rewarding for me. First, define the uniform average of the controls as:

$$
\bar{\mathbf{y}}_0 = \frac{1}{N_0} \sum_{j=1}^{N_0} \mathbf{y}_0^{(j)}
$$
...
Now, we can expand expand the squared Euclidean norm:

$$
f(\beta) = \sum_{t=1}^{T_0} (y_{1t} - \bar{y}_{0t} - \beta)^2.
$$

To find the value of $\beta$ that minimizes the objective, we take the derivative of $f(\beta)$ with respect to $\beta$. We'll need the [power rule](https://www.youtube.com/watch?v=z1lwai-lIzY) and [chain rule](https://www.youtube.com/watch?v=XIQ-KnsAsbg).
...
We want to compute its derivative with respect to $\beta$:
...
In our case, the outer function is $g(u) = u^2$ and the inner function is $h(\beta) = u_t(\beta) = y_{1t} - \bar{y}_{0t} - \beta$.

Differentiating the outer function with respect to $u$ gives
...
Okay chain rule over! Now we can set the derivative equal to zero:

$$
-2\sum_{t=1}^{T_0} (y_{1t} - \bar{y}_{0t} - \beta^\ast) = 0
$$

and solve. Divide both sides by $-2$ (which does not affect the equality):

$$
\sum_{t=1}^{T_0} (y_{1t} - \bar{y}_{0t} - \beta) = 0.
$$

Distribute the sum over subtraction:

$$
\sum_{t=1}^{T_0} (y_{1t} - \bar{y}_{0t}) - \sum_{t=1}^{T_0} \beta = 0.
$$

$\beta$ does not depend on $t$. By linearity of summation, and since $\beta$ has no variable attatched to it, this is equivalent to doing $\sum_{t=1}^{T_0} \beta = \beta + \beta + \cdots + \beta = T_0 \beta.$ This gives us:
...
Thus, the optimal $\beta^\ast$ is simply the average pre-treatment difference between the treated unit and the control group.
...
The words "real control group" are doing **a lot** of heavy lifting here. DID does not have "a real control group" any more than SCM does; the control group is a decision made by the econometrician at runtime, and they are not passed down from the heavens. In other words, the control group employed is a deisng decision, one which you must be prepared to made and defend. Beyond this, weights are a feature (not a bug) of how we compute treatment effects and iROAS. The SCM weights themselves are not a black box fabrication that is somehow less based in reality than DID’s weights or some other method, they're derived from an optimization problem. DID's imposition of uniform weights, if anyhting, is a prior of sorts that must be defended. There’s nothing more real/valid about DID’s uniform weights than those we could obtain from SCM or even nearest neighbor matching. The main question then, for the applied researcher, is what structure should impose upon the weights, and when we should prefer one set of weights to another. In the case of an RCT, we count on randomization to give us unconfoundedness. In DID, we rely on the assumption of parallel trends. In SCM, we choose a combination of controls that best approximates the treated unit in the pre-intervention period. In the end, the best weights are those whose identification assumptions can best be defended.
