...
Now let me translate all of that into the language of the [last post](averagesexcp.html), where every summary statistic turned out to be an argmin. The bridge is one identity we proved there: a (weighted) mean is the constant that minimizes (weighted) squared error. In the population,
...
$$
\min_{\mathbf{w}, \, \beta} \; \big\| \mathbf{y}_1 - \mathbf{Y}_0 \mathbf{w} - \beta \mathbf{1} \big\|_2^2
\qquad \text{subject to} \qquad
w_j \ge 0, \quad \textstyle\sum_{j \in \mathcal{N}_0} w_j = 1 .
$$
...
$$
\min_{\beta} \big\| \mathbf{y}_1 - \mathbf{Y}_0 \mathbf{w} - \beta \mathbf{1} \big\|_2^2
= \big\| \mathbf{M}\mathbf{y}_1 - \mathbf{M}\mathbf{Y}_0 \mathbf{w} \big\|_2^2 .
$$
