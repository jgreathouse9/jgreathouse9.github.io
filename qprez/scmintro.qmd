...
where $\mathcal{W}_{\mathrm{conv}} = \{\mathbf{w}^{\mathrm{SCM}} \in \mathbb{R}_{\ge 0}^{N_0} \mid \mathbf{1}^\top \mathbf{w} = 1\}$. This finds the weight vector $\mathbf{w}^{\mathrm{SCM}}$ that best matches the treated unit’s pre-treatment outcomes as a weighted average of donor outcomes, subject to the counterfactial predictions being within the span of the donor pool, favoring control units with the highest convex relation to the treated unit.



## SCM for Basque

When we run the optimization, we get that Basque Country is best reproduced by 85% Catalonia and 15% Madrid.
