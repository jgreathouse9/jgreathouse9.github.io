...
$$
\hat{\mathbf{y}}_1^{\text{SC}} = \mathbf{Y}_0^{\text{post}} \mathbf{w}^\ast.
$$

The estimated treatment effect at each post-treatment time point is then given by the difference between observed and counterfactual outcomes: $\hat{\tau}_{1,t} = y_{1,t} - \hat{y}_{1,t}$ for $t \in \mathcal{T}_2$. Of course, the method is predicated on good preatment fit, where the predictions in the pre-treatment period should ideally have a low root mean squared error with respect to the treatment unit's observed values.
...
$\mathbf{u}_{\text{pre}} = \mathbf{y}_{1,\text{pre}} - \hat{\mathbf{y}}_{1,\text{pre}}$.   Let $\hat{\sigma}^2 = \frac{1}{T_0 - 1} \left\| \mathbf{u}_{\text{pre}} - \bar{u} \mathbf{1} \right\|^2$ be the unbiased estimator of the residual variance, where $\bar{u} = \frac{1}{T_0} \sum_{t=1}^{T_0} u_t$ is the mean residual. We aim to construct prediction intervals for the counterfactual outcomes in the post-treatment period, accounting for uncertainty due to the unobserved post-treatment error term. Let $\hat{\mathbf{y}}_{1,\text{post}} \in \mathbb{R}^{T_1}$ be the post-treatment SC predictions, where $T_1 = T - T_0$. Assuming that the out-of-sample error is conditionally sub-Gaussian (in English, that large errors are unlikely) given the history $\mathscr{H}$, we obtain a valid non-asymptotic prediction interval via the concentration inequality:   $\delta_\alpha = \sqrt{2 \hat{\sigma}^2 \log(2 / \alpha)}$. The conformal-style prediction intervals are then defined as   $\mathbf{l}_{\text{post}} = \hat{\mathbf{y}}_{1,\text{post}} - \delta_\alpha \mathbf{1}$,   $\mathbf{u}_{\text{post}} = \hat{\mathbf{y}}_{1,\text{post}} + \delta_\alpha \mathbf{1}$. These bounds provide uniform coverage guarantees under the sub-Gaussian assumption on the prediction error.
...
To begin, I conduct a simple synthetic study. Suppose we are working at Airbnb, and we wish to see the causal effect of the introduction of [Airbnb Experiences](https://news.airbnb.com/airbnb-partners-with-art-basel-miami-beach/) on [Gross Booking Value](https://www.lodgify.com/encyclopedia/gross-booking-revenue/) (GBV), a metric which is defined as ''the total revenue generated by room or property rentals before any costs or expenses are subtracted''. Well, in theory, Airbnb Expereinces may make the use of the Airbnb more attractive to tourists, since now tourists no longer need use Viator or other booking services to find stuff to do locally. In other words, it makes it an attraction to that city specifically, and we may see an increase in GBV as a result. Well, all sorts of things may be related to this metric, such as local hotel prices, preexisting level of tourist arrivals, average city-specific booking price, and other relevant metrics. The goal is to see how the GBV would have evolve absent the policy.

I simulate a panel dataset consisting of $N$ units each observed over $T = T_0 + T_1$ time periods, with $K$ distinct outcomes. For each unit $i \in \{1, \dots, N\}$, time $t \in \{1, \dots, T\}$, and outcome $k \in \{1, \dots, K\}$, the observed outcome $\mathbf{Y}_{i,t,k}$ evolves according to an autoregressive process with latent structure: 

$$
\mathbf{Y}_{i,t,k} = 
\rho_k \mathbf{Y}_{i,t-1,k} +
(1 - \rho_k) \left( 
\alpha_{i,k} + \beta_{t,k} + \boldsymbol{\phi}_i^\top \boldsymbol{\mu}_{t,k} + \mathbf{S}_{i,t} + \delta_k
\right) + \varepsilon_{i,t,k}, \quad \text{for } t > 1,
$$
...
$$
\mathbf{Y}_{i,1,k} = 
\alpha_{i,k} + \beta_{1,k} + \boldsymbol{\phi}_i^\top \boldsymbol{\mu}_{1,k} + \mathbf{S}_{i,1} + \delta_k + \varepsilon_{i,1,k}.
$$

Here, $\alpha_{i,k} \sim \mathcal{N}(0, 1)$ and $\beta_{t,k} \sim \mathcal{N}(0, 1)$ represent unit-outcome and time-outcome fixed effects, respectively. Each unit $i$ possesses latent attributes $\boldsymbol{\phi}_i \in \mathbb{R}^r \sim \mathcal{N}(0, \mathbf{I})$, while each time-outcome pair $(t, k)$ has associated latent loadings $\boldsymbol{\mu}_{t,k} \in \mathbb{R}^r \sim \mathcal{N}(0, \mathbf{I})$. The seasonal component $\mathbf{S}_{i,t}$ captures unit-specific periodicity and is defined as $\gamma_i \cos\left( \frac{4\pi(t - \tau_i)}{T_{\text{season}}} \right)$, with $\gamma_i \sim \text{Unif}(0, \bar{\gamma})$ representing the amplitude and $\tau_i \sim \text{Unif}\{0, \dots, T_{\text{season}} - 1\}$ the phase shift. Each outcome $k$ has a baseline shift $\delta_k \sim \text{Unif}(200, 500)$, an autocorrelation parameter $\rho_k \in (0, 1)$, and an idiosyncratic noise component $\varepsilon_{i,t,k} \sim \mathcal{N}(0, \sigma^2)$. One unit is designated as treated—specifically, the unit with the second-largest realization on the first latent factor dimension—and receives an additive treatment effect of $+5$ to the primary outcome (gross booking value) during all post-treatment periods $t > T_0$.
