...
line_hatY, = ax.plot(
    time,
    w_start[0]*Y0[:,0] + w_start[1]*Y0[:,1], 
    label="Synthetic Basque", color="blue", linestyle="--", linewidth=2
)
...
# Dynamic shaded ATT gap (initialize empty)
att_shade = ax.fill_between(time[T0:], y[T0:], y[T0:], color="gray", alpha=0.3, label="ATT Gap")
...
def update(frame):
    ...
    # Update shaded ATT region
    for coll in att_shade.get_paths():  # clear old shading
        coll.vertices[:] = []
    att_shade.remove()
...
extra_frames = 50  # hold final state ~5s at 10 fps
