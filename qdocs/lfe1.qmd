...
We will compute the synthetic control weights by hand, and then show how we get the same solution in `cvxpy.`

## Deriving the Weights

Suppose we have a treated unit and two donor units in 2D:
...
Without getting too far off track, we can use this to frame the synthetic control problem as projecting $\mathbf{y}$ onto the convex hull $C$ of the donors $\{\mathbf{y}_1, \mathbf{y}_2\}$, which is the line segment between $\mathbf{y}_1$ and $\mathbf{y}_2$. The projection $\hat{\mathbf{y}}$ minimizes the Euclidean distance to $\mathbf{y}$, and the residual is orthogonal (or obtuse) to directions within $C$. The function to minimize is:
...
We can begin by simplifying. Factoring out the common term $2 \cdot 28 = 56$, we get

$$
(2 + 28 w_1) + (10 + 28 w_1) = 0 \quad \implies \quad 12 + 56 w_1 = 0.
$$

Solving for $w_1$ gives the unconstrained minimizer

$$
w_1 = -\frac{12}{56} = -\frac{3}{14}.
$$
...
Geometrically, this corresponds to projecting the treated vector $\mathbf{y}$ onto the line segment defined by the donor units. The residual vector is

$$
\mathbf{r} = \mathbf{y} - \hat{\mathbf{y}} = \begin{bmatrix} 2 \\ 10 \end{bmatrix},
$$

and the direction spanning the convex set from $\hat{\mathbf{y}}$ is

$$
\mathbf{y}_1 - \mathbf{y}_2 = \begin{bmatrix} -28 \\ -28 \end{bmatrix}.
$$

The Hilbert Projection Theorem guarantees that the residual is obtuse (or orthogonal) to all directions within the convex set. Indeed, the dot product

$$
\langle \mathbf{r}, \mathbf{y}_1 - \mathbf{y}_2 \rangle = 2(-28) + 10(-28) = -336 \le 0
$$

confirms that moving toward $\mathbf{y}_1$ would increase the distance, so $\hat{\mathbf{y}} = \mathbf{y}_2$ is indeed the projection onto the closest face of the convex hull, exactly as predicted by the Hilbert Projection Theorem.

This result is consistent with the [Minkowski-Weyl theorem](https://www.youtube.com/watch?v=RquUIXUMLcc)

::: {.theorem #minkowski}
**Minkowski-Weyl Theorem.** Let $C \subset \mathbb{R}^T$ be a compact convex set. Then every point in $C$ can be expressed as a convex combination of extreme points of $C$.
:::

The convex set $C$ is the compact line segment between $\mathbf{y}_1$ and $\mathbf{y}_2$, with extreme points $\mathbf{y}_1$ and $\mathbf{y}_2$. The projection $\hat{\mathbf{y}} = \mathbf{y}_2$ is itself an extreme point, expressed as the convex combination with weights $w_1 = 0$, $w_2 = 1$.

Or, every point in a compact convex set can be expressed as a convex combination of its extreme points.

If we wish for an algebraic interpretation of this, we can always use the [Karush-Kuhn-Tucker](https://mdav.ece.gatech.edu/ece-3803-fall2021/notes/17-notes-3803-f21.pdf) conditions.

::: {.theorem #kkt}
**KKT Optimality Conditions.** At the optimum $\mathbf{w}^{\ast}$, there exist Lagrange multipliers $\lambda \in \mathbb{R}$ (for equality) and $\mu \ge 0$ (for inequality) such that:

$$
2 \mathbf{Y}^\top (\mathbf{Y} \mathbf{w}^{\ast} - \mathbf{y}) + \lambda \mathbf{1} - \mu = 0, \quad
\mu_i w_i^{\ast} = 0, \quad i=1,\dots,N.
$$
:::

Here is our general optimization problem:

$$
\min_{\mathbf{w} \in \mathbb{R}^2} \|\mathbf{y} - \mathbf{Y} \mathbf{w}\|^2, \quad \text{s.t.} \quad w_1, w_2 \ge 0, \quad w_1 + w_2 = 1.
$$

For the candidate $\mathbf{w}^{\ast} = \begin{bmatrix} 0 \\ 1 \end{bmatrix}$, compute $\mathbf{Y} \mathbf{w}^{\ast} = \mathbf{y}_2 = \begin{bmatrix} 48 \\ 50 \end{bmatrix}$, so:
...
$$
\mathbf{Y}^\top (\mathbf{Y} \mathbf{w}^{\ast} - \mathbf{y}) = \begin{bmatrix} 20 & 22 \\ 48 & 50 \end{bmatrix} \begin{bmatrix} -2 \\ -10 \end{bmatrix} = \begin{bmatrix} 20 \cdot (-2) + 22 \cdot (-10) \\ 48 \cdot (-2) + 50 \cdot (-10) \end{bmatrix} = \begin{bmatrix} -40 - 220 \\ -96 - 500 \end{bmatrix} = \begin{bmatrix} -260 \\ -596 \end{bmatrix}.
$$

The stationarity condition is:

$$
2 \begin{bmatrix} -260 \\ -596 \end{bmatrix} + \lambda \begin{bmatrix} 1 \\ 1 \end{bmatrix} - \begin{bmatrix} \mu_1 \\ \mu_2 \end{bmatrix} = \begin{bmatrix} 0 \\ 0 \end{bmatrix} \implies \begin{bmatrix} -520 + \lambda - \mu_1 \\ -1192 + \lambda - \mu_2 \end{bmatrix} = \begin{bmatrix} 0 \\ 0 \end{bmatrix}.
$$

Complementary slackness: $\mu_1 w_1 = \mu_1 \cdot 0 = 0$, $\mu_2 w_2 = \mu_2 \cdot 1 = 0 \implies \mu_2 = 0$.

From the second equation: $-1192 + \lambda - 0 = 0 \implies \lambda = 1192$.

From the first equation: $-520 + 1192 - \mu_1 = 0 \implies \mu_1 = 672 \ge 0$.

All conditions hold: stationarity, primal feasibility ($w_1^{\ast} \ge 0$, $w_2^{\ast} \ge 0$, sum to 1), dual feasibility ($\mu \ge 0$), and complementary slackness. Thus, $\mathbf{w}^{\ast} = \begin{bmatrix} 0 \\ 1 \end{bmatrix}$ is optimal.

We have already verified using the KKT conditions that the optimal weights for this example are

$$
\mathbf{w}^{\ast} = \begin{bmatrix} 0 \\ 1 \end{bmatrix},
$$

with Lagrange multipliers $\mu_1 = 672$, $\mu_2 = 0$, and $\lambda = 1192$. These multipliers quantify how the constraints influence the solution: the nonnegativity of $w_1$ is active, which is why $\mu_1 > 0$, while $w_2$ is strictly positive and unconstrained, resulting in $\mu_2 = 0$. The stationarity condition ensures that the gradient of the objective function is exactly balanced by the constraints, while complementary slackness explains why only the active constraint has a nonzero multiplier. Primal and dual feasibility confirm that the candidate solution satisfies both the original weight constraints and the nonnegativity of the multipliers. Taken together, the KKT conditions rigorously establish that this choice of weights is optimal.

Geometrically, this optimization problem can be viewed as projecting the treated vector $\mathbf{y}$ onto the convex hull of the donor vectors. The Hilbert Projection Theorem guarantees that there is a unique closest point within the convex hull and that the residual vector, defined as the difference between the treated vector and the projection, forms an obtuse angle or is orthogonal with respect to all directions inside the convex set. In our case, the unconstrained least squares solution would have required $w_1 < 0$, which lies outside the feasible convex hull. The KKT conditions formalize this by showing that the constraint $w_1 \ge 0$ is active, enforcing the feasible solution algebraically. From the geometric perspective, HPT explains that because the unconstrained minimum is infeasible, the closest feasible point lies on the boundary of the convex hull, specifically at the extreme point $\mathbf{y}_2$.
