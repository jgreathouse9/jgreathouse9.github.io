...
Exploiting the fact that summation is a linear operator, it distributes over addition and allows constants to factor out. Since $\mu$ does not at all depends on $i$, it appears identically in every term of the sum. Beyond, this is exactly what we seek: one single value that is as close as possible to all the data points simultaneously
