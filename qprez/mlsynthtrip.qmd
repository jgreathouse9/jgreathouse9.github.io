...
- **The solution**: `mlsynth` — a lightweight Python library unifying over 20 SCMs under one consistent grammar, meaning analysts may plug and play different methods quickly instead of needing to manually code others.

- **The impact**:
  - Run multiple SCMs in a few minutes with less than 40 lines of code
...
- However, several SCM advances that Tripadvisor may benefit from have non-trivial startup costs to using them effectively.

- The first cost is purely software: not all methods are offered in one software, not all software are free.

- This makes testing different SCMs in a single workflow difficult, especially given that other existing libraries differ in terms of data input expectations, options, and dependencies.
...
**Tripadvisor impact**: Manual/ad-hoc processes in Python/R slow implementation SCMs for business-critical use cases.
...
- Integrates seamlessly into GitHub Actions or CI/CD pipelines
...
- **Objective**: We find the moratorium increase hotel prices by roughly 12 percent.

---

## Using `mlsynth`

This chart shows the moratorium's increased the prices of hotel rooms by ~12%.
...
- **Before**: Manual R workflows, fragmented syntaxes, potentially hours of setup
...
- `mlsynth` removes barriers to using advanced SCMs with a unified, Python-native grammar
