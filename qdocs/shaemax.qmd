...
Which estimators run is controlled by the `methods` list: you pass the names of the corner-case estimators you want, and `RESCM` fits each one. The relaxed-balance estimators are `RELAX_L2` (Euclidean relaxation), `RELAX_ENTROPY` (entropy relaxation), and `RELAX_EL` (empirical-likelihood relaxation). The relaxation strength is the `tau` parameter, which can be provided explicitly (in which case no cross-validation is performed) or left as `None` to be selected by time-series cross-validation over a grid of candidate values. The number of candidate taus is controlled by `n_taus`, and the number of cross-validation folds by `n_splits`.

The two elastic-net estimators combine an L1 penalty with a second norm on the donor weights: `ENET` pairs L1 with $\ell_2$, and `L1LINF` pairs L1 with $\ell_\infty$. Their mixing parameter $\alpha$ (the weight on L1 versus the second norm) and overall penalty strength $\lambda$ are chosen by the same cross-validation when they are not fixed. The classical corners `SC`, `LASSO`, `RIDGE`, and `LINF` are available as method names too, and lower-case aliases (`l2`, `entropy`, `el`, `enet`, `linf`) are accepted. The first method you list drives the convenience aliases on the returned results object.
