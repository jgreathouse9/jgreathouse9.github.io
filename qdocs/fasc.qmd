...
The FASC estimator combines two complementary ideas: sparse donor selection via forward selection and bias correction via affine augmentation. It begins with FSCM, which produces a base weight vector $\mathbf{w}_{S^\ast}^\ast$. Let $\mathcal{S}_0 \subseteq \mathcal{N}_0$ denote the donor indices selected by the FSCM corresponding to the non-zero entries of $\mathbf{w}_{S^\ast}^\ast$. The full donor weight vector is then defined componentwise as

$$
w_j^{\mathrm{full}} = (\mathbf{w}_{S^\ast}^\ast)_j \, \mathbf{1}_{\{j \in \mathcal{S}_0\}}, \quad j \in \mathcal{N}_0,
$$
