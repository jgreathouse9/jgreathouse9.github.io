...
- **Connection to Difference-in-Differences (DiD)**    
  - DiD assumes outcomes follow a simple factor model:

    $$
    y_{jt}(0) = \mathbf{\mu}_j + \mathbf{\lambda}_t + \epsilon_{jt}
    $$
    where $\mathbf{\mu}_j \in \mathbb{R}$ are unit fixed effects and $\mathbf{\lambda}_t \in \mathbb{R}$ are time fixed effects.  

  - SCM generalizes this with an *interactive fixed effects model*:  
    $$
...
Here is our prediction using OLS. Unfortunately, while OLS fits very well to the Basque Country before terrorism happened, the out-of-sample/post-1975 predicions exhibits a lot moer variance than any control unit in the pre-treatment period did. The counterfactual has a weird zig-zag-y prediction. We can see that the prediction line for the Basque country suggests that its economy would have fallen off a cliff, had terrorism **not** happened. In other words, if the OLS estmate is to be taken seriously, the onset of terrorism saved the Basque economy... This does not seem like a sensible finding historically, or a very reasonable prediciton statistically
...
## Assumptions of the Classic Synthetic Control Method

- **Good pre-treatment fit is essential**: SCM relies on the treated unit being well approximated by a weighted combination of donors before the intervention. This suggests we need a low root-mean squared error for the fitted pre-treatment predictions. 

- **SUTVA (no interference)**: The treated unit’s outcome should not be affected by other units’ treatments. If spillovers are possible, one approach is: first, estimate synthetic controls for all "possibly affected" units using "certainly unaffected" units; then use these synthetic outcomes as donors for the main unit of interest.

- **Convex combination assumption**: The treated unit must be representable as a convex combination of donor units. That is, the weights $\mathbf{w}^{\mathrm{SCM}}$ satisfy

  $$
  \mathbf{w}^{\mathrm{SCM}} \ge 0, \quad \mathbf{1}^\top \mathbf{w}^{\mathrm{SCM}} = 1
  $$

- **No anticipation:** The treated unit does not adjust its behavior before the intervention in anticipation of treatment. If a policy is announced in a city, but is not implemented yet until later, consisder using the announcement treatment date as the date of treatment, instead of the day the policy begins.
...
## Key Takeaways for Practitioners (with mathematical conditions)
...
- **Low-volatility series**  
  ...
  *Condition (informal):* the pre-treatment volatility should be small relative to the signal, e.g.
  $$
  \frac{\operatorname{sd}(y_{1,t\le T_0})}{\overline{y}_{1,t\le T_0}} \quad\text{is small.}
  $$
...
- **Small, relevant donor pool**  
  More donors can increase interpolation bias and overfitting. Prefer a focused donor set.  
  *Condition (selection):* restrict donor count $J$ relative to pre-period information, e.g.
  ...
  $$
  and choose donors that minimize pre-fit distance:
  $$
  \min_{\mathcal{D}\subseteq\{1,\dots,J\}} \; \operatorname{dist}\big(y_{1,t\le T_0},\, \{y_{j,t\le T_0}: j\in\mathcal{D}\}\big).
  $$
...
- **Sparsity aids interpretability**  
  ...
  *Condition:* prefer weight vectors $\mathbf{w}$ with small support:
  $$
  \|\mathbf{w}\|_0 \text{ small} \quad\text{or enforce } \|\mathbf{w}\|_1 \text{ penalties to induce sparsity.}
  $$
...
- **Plot your treated unit versus the donors (seriously)**  
  ...
  *Diagnostic:* inspect $\{y_{j,t}\}_{j\in\text{donors}}$ vs.\ $y_{1,t}$ for $t\le T_0$.

- **Good pre-treatment fit matters**  
  If pre-fit error is large, post-treatment inferences are unreliable.  
  *Condition (quantitative):* require a low pre-fit RMSE or relative RMSE:
  $$
  \text{RMSE}_{\text{pre}} = \sqrt{\frac{1}{T_0}\sum_{t=1}^{T_0}\big(y_{1t}-\hat y_{1t}\big)^2} \quad\text{is small,}
  $$
  or a small relative fit error
  $$
  \frac{\text{RMSE}_{\text{pre}}}{\overline{y}_{1,t\le T_0}} \text{ is small.}
  $$

- **Out-of-sample validation is essential**  
  Use placebo tests (in-time and in-space) and permutation inference to check robustness.  
...
*Note:* these conditions are qualitative guides rather than strict thresholds. In practice, combine the quantitative checks above (RMSE, volatility, placebo p-values) with visual inspection to decide whether an SCM analysis is credible.
