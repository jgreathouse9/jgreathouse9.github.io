...
gdf.plot(ax=ax, color='#f0f0f0', edgecolor='white', linewidth=0.8)
...
We index time by $t \in \{1,\dots,T\}$ and units by $j \in \mathcal{J} = \{1,\dots,J+1\}$, where unit 1 is treated and the others are controls.
...
- $\mathcal{J} = \{1,\dots,J+1\}$: set of all units (1 treated + $J$ controls)  
- $\mathcal{J}_0 = \mathcal{J} \setminus \{1\} = \{2,\dots,J+1\}$: set of untreated donor units, with cardinality $|\mathcal{J}_0| = J$
- $\mathbf{y}_1 \in \mathbb{R}^{T}$: outcome vector (e.g., GDP per capita) for the **treated unit** (e.g., Basque Country) over all $T$ time periods  
