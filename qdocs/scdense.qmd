...
$$
Let $t \in \mathbb{N}$ and $i \in \mathbb{N}$, represent the indices for $T$ time periods and  $N$ units. The pre-treatment period consists of consecutive time periods $\mathcal{T}_1 = \{1, 2, \ldots, T_0\}$, while the post-treatment period is given by $\mathcal{T}_2 = \{T_0 + 1, \ldots, T\}$. The treated unit is indexed by $i = 1$, while the remaining set of units, $\mathcal{N}_0 \coloneqq \{i = 2, \ldots, N_0 + 1\}$, forms the control group. Each outcome for all units is denoted by $y_{it}$. The key challenge of causal inference is that for each unit $i$ and time $t$, we only observe the factual outcome, expressed as:  
$$
