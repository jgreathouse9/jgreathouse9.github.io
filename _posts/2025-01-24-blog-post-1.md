...
Our goal is to estimate the counterfactual, or $y_{1t}^{0}$. These are the values of an outcome for a treated unit we would see if some intervention was never done. The difficulty however is that we can see units as treated or not, $y_{it}=y_{1t}^{1}d_{it}+\left(1-d_{it}\right)y_{1t}^{0}$. 
