...
$$
\mathbf{w}^\ast = \underset{\mathbf{w} \in \Delta^{N_0}}{\operatorname*{argmin}} \|\mathbf{y}_1 - \mathbf{Y}_0 \mathbf{w} \|_2^2.
$$

We find the weight vector, $\mathbf{w}$, that minimizes the mean squared error between the treated unit's pre-treatment outcomes and the in-sample weighted average of control units. For our purposes, the space of synthetic control weights is the $N_0$-dimensional probability simplex:

$$
\Delta^{N_0} = \left\{ \mathbf{w} \in \mathbb{R}_{\geq 0}^{N_0} : \|\mathbf{w}\|_1 = 1 \right\}.
$$

Practically this means that the predictions will never be outside the donor pool's outcome values. The key thing is choosing the right donor units such that we have better in-sample fit.

## Forward Selection for Donor Pool Choice

Consider a restricted donor pool chosen by the forward selection algorithm. This consists of a subset of control units, $\mathcal{S} \subseteq \mathcal{N}_0$, with cardinality $k = |\mathcal{S}|$ where $k \leq N_0$. This subset induces a subsimplex of weights:

$$
\Delta^{k}(\mathcal{S}) = \left\{ \mathbf{w}^\prime \in \mathbb{R}_{\geq 0}^{k} : \|\mathbf{w}^{\prime}\|_1 = 1 \right\},
$$
where ideally some control units will be more relevant to the treated unit than others. The forward selection algorithm, over $K \in \mathbb{N}$ iterations, builds a sequence of tuples, $\mathbb{T} = \{(\mathcal{S}_K, \text{MSE}_K) \}_{K=1}^{N_0}$. The tuple contains the selected donor set for the $K$-th iteration and its corresponding $\text{MSE}_K$ (or the pre-treatment mean squared error). We begin in the first step by minimizing the SCM objective function as above, cycling through each donor unit one at a time. This returns $N_0$ one unit SCM submodels. We choose the single unit (the nearest neighbor in this specific case) that minimizes the MSE with respect to the treated unit. Our first tuple, then, is this single unit and its corresponding MSE

$$
\mathcal{S}_1 = \{j^\ast\}, \quad \text{where} \quad j^\ast = \underset{j \in \mathcal{N}_0}{\operatorname*{argmin}} \ \text{MSE}(\{j\}).
$$

This process continues for the rest of the donor pool. For $K=2$, we see which set of two donor units, the first one included, minimizes the $\text{MSE}$. As before, this becomes our second selected donor unit and MSE

$$
 \mathcal{S}_2 = \mathcal{S}_1 \cup \{j^\ast\},\quad j^\ast = \underset{j \in \mathcal{N}_0 \setminus \mathcal{S}_{K-1}}{\operatorname*{argmin}} \ \text{MSE}(\mathcal{S}_{K-1} \cup \{j\}),
$$

with the updated tuple taking the form of

$$
(\mathcal{S}_K, \text{MSE}_K) = (\mathcal{S}_{K-1} \cup \{j^\ast\}, \text{MSE}(\mathcal{S}_{K-1} \cup \{j^\ast\})).
$$

This process continues until all $N_0$ donors have been included in the SCM model. The final donor set we use for analysis is the tuple with the lowest $\text{MSE}$:

$$
\mathcal{S}^{\ast} = \underset{(\mathcal{S}_K, \text{MSE}_K) \in \mathbb{T}}{\operatorname*{argmin}} \ \text{MSE}_K.
$$

$\mathcal{S}^{\ast}$ is the optimal donor pool by forward selection. Note that even within $\mathcal{S}^{\ast}$, some donors may receive zero weight in the final solution, as these are just the units selected for inclusion in the donor pool. They are not necessarily the ones that will actually get weight, in contrast to methods such as [Forward Difference-in-Differences](https://doi.org/10.1287/mksc.2022.0212) or the [forward selection panel data method](https://doi.org/10.1016/j.jeconom.2021.04.009), both of which are available in `mlsynth`.
