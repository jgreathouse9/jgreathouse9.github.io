...
Here is the results of the ```CLUSTERSC``` class. This class implements two SCMs which use k-means clustering and low-rank methods to select the donor pool (Principal Componenet Regression and Robust PCA specifically). Both select 25 of the 30 donor units.
...
Here we estimate the Forward DID method. The selction algorithm is spelt out in the paper, but we baiscally add a new unit to the DID control group if the new unit increases the pre-treatment $R^2$ statistic. FDID chooses Anhui, Beijing, Fujian, Henan, Hunan, Jiangsu, Yunnan, Zhejiang. This group gives a pre-treatment $R^2$ of 0.996.
