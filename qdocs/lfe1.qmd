...
For a final expression, we can also arrive at the solution via pure geometry. We seek to approximate
...
Thus, the Frank–Wolfe geometric approach, the calculus, and the KKT system are three perspectives on the same underlying fact: the optimal synthetic control is the projection of $\mathbf{y}$ onto the convex hull of donors, and when the unconstrained minimizer lies outside the hull, the solution is constrained to the nearest boundary point, with the residual aligned according to the Hilbert Projection Theorem. 
...
Here is the result we get when we fit the TSSC estimator to the Australia case.
