...
In this case we have only two units. So it helpsus to define one weight in terms of the other. Substituting $w_2 = 1 - w_1$:
...
However, this value of $w_1$ lies outside the weight constraint $w \in [0,1]$. Since this is outside the feasible set $[0,1]$, we assign $w_1$ the closest legal weight of 0. Now we can evaluate the objective function at the boundaries:
...
KKT is useful here because it provides a systematic way to check all the optimality conditions at once—stationarity, feasibility, dual feasibility, and complementary slackness. This guarantees that the solution we find is not just locally valid but globally optimal for a convex quadratic problem. In this example, KKT confirms that the active constraint is $w_1 \ge 0$, leading to the boundary solution $w^\ast = (0,1)$.
...
From calculus, we expect the constrained minimizer to be $\mathbf{w}^* = (0,1)^\top$, which corresponds to taking donor two alone as the synthetic control. Verifying this candidate requires computing the residual at that point and solving the stationarity equations for the multipliers. At $\mathbf{w}^*$ we have $\mathbf{Y}\mathbf{w}^* = \mathbf{y}_2 = (48,50)^\top$ and the residual $\mathbf{Y}\mathbf{w}^* - \mathbf{y} = (-2,-10)^\top$. Multiplying by $2 \mathbf{Y}^\top$ yields

$$
2 \mathbf{Y}^\top (\mathbf{Y}\mathbf{w}^* - \mathbf{y}) = 2 \begin{bmatrix}20 & 22 \\ 48 & 50\end{bmatrix} \begin{bmatrix}-2 \\ -10\end{bmatrix} = 2 \begin{bmatrix}-260 \\ -596\end{bmatrix} = \begin{bmatrix}-520 \\ -1192\end{bmatrix}.
$$
...
Complementary slackness applied to the candidate $w_1^* = 0, w_2^* = 1$ implies $\mu_2 = 0$ because $w_2^* > 0$ forces the corresponding multiplier to vanish, while $\mu_1$ is unconstrained by slackness other than being nonnegative. Using $\mu_2 = 0$ in the second scalar equation yields $\lambda = 1192$. Substituting $\lambda = 1192$ into the first equation gives $\mu_1 = 672$, which satisfies dual feasibility since $672 \ge 0$. Thus stationarity, primal feasibility, dual feasibility, and complementary slackness all hold for the candidate, and because the objective is convex (its Hessian is $2 \mathbf{Y}^\top \mathbf{Y}$, which is positive semidefinite) these KKT conditions guarantee global optimality. Therefore $\mathbf{w}^* = (0,1)^\top$ is the solution, the residual is $\mathbf{y} - \hat{\mathbf{y}} = (2,10)^\top$, the Lagrange multiplier for the equality constraint is $\lambda = 1192$, and the dual variables are $\mu_1 = 672$, $\mu_2 = 0$.
...
Now for the pure geometry. We again want to approximate $\mathbf{y} = \begin{bmatrix}50\\60\end{bmatrix}$ using a convex combination of donors $\mathbf{y}_1 = \begin{bmatrix}20\\22\end{bmatrix}$ and $\mathbf{y}_2 = \begin{bmatrix}48\\50\end{bmatrix}$. The feasible set of synthetic predictions is the convex hull $\operatorname{conv}\{\mathbf{y}_1, \mathbf{y}_2\}$, i.e., the line segment connecting the two donors. Minimizing the squared distance $\|\mathbf{y} - \hat{\mathbf{y}}\|^2$ is equivalent to projecting $\mathbf{y}$ onto this segment.

The Frank–Wolfe algorithm formalizes this as an iterative procedure. It starts at the donor closest to $\mathbf{y}$ in Euclidean distance. Computing the squared distances, we find

$$
\|\mathbf{y} - \mathbf{y}_1\|^2 = 2344, \quad \|\mathbf{y} - \mathbf{y}_2\|^2 = 104,
$$

so $\mathbf{y}_2$ is the initial point $x^{(0)}$, with weights $w^{(0)} = (0,1)$. The gradient of the objective $f(x) = \frac{1}{2}\|\mathbf{y}-x\|^2$ at $x^{(0)}$ is $x^{(0)} - \mathbf{y} = \begin{bmatrix}-2\\-10\end{bmatrix}$. Frank–Wolfe then chooses a vertex $s \in \{\mathbf{y}_1,\mathbf{y}_2\}$ that minimizes the inner product $\langle \nabla f(x^{(0)}), s \rangle$. Computing

$$
\langle \nabla f(x^{(0)}), \mathbf{y}_1 \rangle = -260, \quad \langle \nabla f(x^{(0)}), \mathbf{y}_2 \rangle = -596,
$$

the minimum occurs at $\mathbf{y}_2$, meaning the Frank–Wolfe search direction points toward $\mathbf{y}_2$—we are already there. Attempting to move toward $\mathbf{y}_1$, the direction is $d = \mathbf{y}_1 - x^{(0)} = \begin{bmatrix}-28\\-28\end{bmatrix}$, and the optimal step along this direction is

$$
\gamma^* = \frac{\langle \mathbf{y}-x^{(0)}, d \rangle}{\|d\|^2} = -0.214,
$$

which is outside the feasible range $[0,1]$, so it is clamped to $\gamma=0$. The update $x^{(1)} = x^{(0)} + \gamma d = \mathbf{y}_2$ confirms that the closest feasible point is the boundary vertex $\mathbf{y}_2$, with weights $w^* = (0,1)$ and residual $\mathbf{r} = \mathbf{y} - \hat{\mathbf{y}} = \begin{bmatrix}2\\10\end{bmatrix}$.

This geometric reasoning mirrors the KKT derivation. In the KKT solution, the unconstrained minimizer $w_1 = -3/14$ lay outside the feasible $[0,1]$ interval, forcing the solution onto the boundary $w_1 = 0$. The KKT multipliers and complementary slackness conditions algebraically enforce that the residual cannot point “into” the feasible set along any direction allowed by the convex hull, exactly paralleling the Frank–Wolfe observation that moving toward $\mathbf{y}_1$ would increase the distance.

The connection to the Hilbert Projection Theorem makes this precise: for any closed convex set $C$ (here, the convex hull of donors) and point $\mathbf{y}$, there exists a unique projection $\hat{\mathbf{y}} \in C$ minimizing $\|\mathbf{y}-\hat{\mathbf{y}}\|$, and the residual satisfies

$$
\langle \mathbf{y}-\hat{\mathbf{y}}, \mathbf{z}-\hat{\mathbf{y}} \rangle \le 0, \quad \forall \mathbf{z} \in C.
$$

In our example, $\mathbf{y}$ lies outside the convex hull line segment, so the projection lands on the nearest endpoint $\mathbf{y}_2$. The residual $\mathbf{r} = \mathbf{y} - \hat{\mathbf{y}}$ forms an obtuse angle with the segment toward $\mathbf{y}_1$, satisfying the Hilbert Projection condition. Both the KKT derivation and the Frank–Wolfe geometric solution are algebraic and iterative realizations of this fundamental projection principle: when the unconstrained solution falls outside the convex hull, the closest feasible point lies on the boundary, producing $w^* = (0,1)$ and residual $\mathbf{y} - \hat{\mathbf{y}} = \begin{bmatrix}2\\10\end{bmatrix}$.

In summary, the Hilbert Projection Theorem unifies the algebraic KKT perspective and the iterative geometric Frank–Wolfe approach: the synthetic control problem is fundamentally about projecting $\mathbf{y}$ onto a convex set of donors, and when the treated unit lies outside that convex hull, the optimal weights correspond to the nearest boundary point.
