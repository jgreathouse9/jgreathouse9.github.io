...
- With a finite set of real numbers  $S = \{x_1, x_2, \dots, x_n\} \subset \mathbb{R}$, the arithmetic mean is:  
...
*Intuition:* ATT measures how much the real, treated Basque GDP deviates from the model (OLS in this case) prediction after terrorism began.
...
for unit, df_placebo in placebo_datasets.items():
    config = {
        ...
        "unitid": unit_col,
        "time": year_col,
        "display_graphs": False,   # suppress if looping
        "save": False, "method": "PCR", "cluster": False
    }
