...
Let $ t \in \mathbb{N} $ and $ i \in \mathbb{N} $, where $ \mathbb{N} = \{1, 2, 3, \ldots\} $, represent the indices for time periods and units, respectively. Each outcome is denoted by $ y_{it} $, where $ y_{it} \in \mathbb{R} $ is the observed outcome for unit $ i $ at time $ t $. The treated unit is indexed as $ i = 1 $, while the remaining $ N_0 $ units ($ i = 2, \ldots, N_0 + 1 $) form the control group. The key challenge of causal inference is that for each unit $ i $ and time $ t $, we only observe the factual outcome, expressed as:  
$$
y_{it} = y_{it}^1 d_{it} + \left(1 - d_{it}\right)y_{it}^0,
$$
where $ y_{it}^1 $ is the potential outcome under treatment, $ y_{it}^0 $ is the potential outcome under no treatment, and $ d_{it} \in \{0, 1\} $ is an indicator for treatment ($ d_{it} = 1 $ if treated, $ d_{it} = 0 $ if untreated).
...
The outcome vector for the treated unit is given by $ \mathbf{y}_1 = \begin{bmatrix} y_{11} & y_{12} & \cdots & y_{1T} \end{bmatrix}^\top \in \mathbb{R}^T $, where each entry corresponds to the outcome of the treated unit at time $ t $. The donor pool matrix, representing the outcomes of the $ N_0 $ control units over all time periods, is defined as  
$$
...
The original SCM estimates the weights $ \mathbf{w} $ by solving the following optimization problem:  
$$
\operatorname{argmin}_{\mathbf{w} \in \mathbb{I}^{N_0}} \lVert \mathbf{y}_1 - \mathbf{w}^\top \mathbf{Y}_0 \rVert_2^2, \quad \text{subject to } \lVert \mathbf{w} \rVert_1 = 1.
$$  
In this formulation, $ \mathbf{y}_1 $ is the pre-treatment outcome vector for the treated unit, and $ \mathbf{w}^\top \mathbf{Y}_0 $ represents the synthetic control, a weighted combination of control unit outcomes. The constraint $ \lVert \mathbf{w} \rVert_1 = 1 $ ensures that the weights sum to one. The objective is to achieve a good pre-treatment fit, such that $ \mathbf{w}^\top \mathbf{Y}_0 \approx \mathbf{y}_1 $, while using the synthetic control to estimate $ y_{1t}^0 $ during the post-treatment period.
