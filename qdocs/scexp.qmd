...
[Suppose the government of Curaçao](https://www.travelpulse.com/news/destinations/curacao-takes-proactive-steps-toward-sustainability-amid-global-overtourism) wishes to implement a new ["Green Stay" initiative](https://www.businesstravelnewseurope.com/Accommodation/HRS-Green-Stay-updates-are-set-to-increase-hotel-participation) that encourages hotels to adopt sustainability measures such as reducing water usage, improving waste management, and shifting toward renewable energy. Such initiatives are becoming [increasingly](https://www.travelandtourworld.com/news/article/aruba-manchebo-beach-resort-and-spa-reinforces-sustainable-practices-now-with-green-globe-platinum-recertification/) [popular](https://www.travelagewest.com/Travel/Caribbean/eco-hotels-caribbean) in recent years.



From a methodological standpoint, the most rigorous way to measure the impact of such a program would be through a randomized controlled trial (RCT), where some hotels are randomly assigned to implement a policy while others serve as controls. Randomization ensures that, *in expectation*, treated and control groups are balanced, and that as a result of that balance whatever differences in outcomes after randomization can be attributed to the policy. RCTs provide a clear framework for statistical inference and is considered the ideal setup in must areas of impact analysis.

Despite these advantages, there are serious obstacles to conducting a true RCT in this context. Ethically, assigning certain hotels to receive the benefits while denying them to others may be perceived as unfair, particularly if the policy boosts reputation, attracts eco-conscious travelers, or leads to financial advantages of some hotels over others. Guests might also experience different standards of service unknowingly, raising questions about fairness and transparency.

Politically, implementing randomization across hundreds/thousands of hotels would require coordination among the government, hotel associations, and local businesses, some of whom may resist being "experimented on" for policy research. Because tourism is central to the island’s economy, public perception would be sensitive, putting it politely, and randomized assignment could easily be framed as a risk to one of Curaçao’s most important industries.

The logistical burden is equally daunting: monitoring and enforcing different sustainability requirements hotel by hotel would be complex and costly, with significant overhead required for compliance and enforcement. In practice, coordinating hundreds of independent hotels is unlikely to succeed. In other words, while an RCT is desirable in theory, ethical, political, and feasibility barriers make it impractical in the context of Curaçao’s hotel sector. This of course generalizes to many practical scenarios where experimentation cannot happen due to simple feasibility reasons.

Given these challenges, a more practical approach is to consider the market at a localized level, grouping neighborhoods into clusters that share similar characteristics, such as geographic location, customer demographics, or historical patterns of tourist activity. By focusing on clusters rather than individual units, we mitigate ethical concerns of treating some hotels differently than others, since selection occurs within naturally similar groups rather than arbitrarily across the whole market.

Cluster analysis also reduces logistical complexity, because interventions can be coordinated at the cluster level rather than needing to monitor and enforce policies across hundreds of disparate hotels. Within each cluster, we can select neighborhoods to be treated that closely resemble the local market and control neighborhoods that reflect the cluster’s underlying dynamics. This ensures that observed effects of the intervention are not driven by unusual outliers or idiosyncratic behaviors of certain units, but rather capture the typical response within a given market segment.

But how would we select which neighborhoods to treat and which to use as controls? Surely we could throw darts at a board, but this likely may may be the best strategy. To address this, [we can use synthetic control methods](https://arxiv.org/abs/2108.02196), which construct "synthetic treated units" and "synthetic control units" that mimic the characteristics of clusters more broadly, enabling us to do treatment effect estimation as a result.
...
In addition to the above, we also may face operational costs $\mathbf{c}$ and cluster budgets $B_k$ which constrain feasible assignments. Thus, the feasible set is
...
Non-negativity and normalization ($w_j, v_j \ge 0$, $\sum w_j = \sum v_j = 1$) mean that synthetic neighborhoods are weighted averages of real neighborhoods. Negative weights would imply "subtracting" a neighborhood’s influence. Summing to one ensures each synthetic neighborhood is in the same scale/mixture of real neighborhoods. Our decision variable $z_j \in \{0,1\}, w_j \le z_j, v_j \le 1 - z_j$ asks whether a unit should be treated. Each neighborhood is either treated or not. A treated neighborhood contributes to the synthetic treated unit but not the control unit, and vice versa, avoiding "double-counting." Practically, analysts may place cardinality constraints $\sum z_j \in [m_{\text{min},k}, m_{\text{max},k}]$ upon the size of the treated group such that at least one neighborhood is treated per cluster to observe effects, but not every neighborhood, respecting ethical, financial, or logistical limits. In a similar vein, budget constraints ($\sum c_j w_j \le B_k$, $\sum c_j w_j \le B_{\text{total}}$ also enter into the optimization, given that some neighborhoods may have different costs to treat.

Abadie and Zhou propose numerous ways of implementing this. The base experimental SC estimator is the most straightforward. For each cluster $k$, it tries to make the synthetic treated and synthetic control neighborhoods look like the cluster mean:
...
This guarantees that both sides of the experiment are anchored to the same benchmark—the average of their cluster. The weakly targeted version extends this idea by also encouraging the treated and control groups to resemble each other directly:
...
In practice, this prevents the optimization from drifting toward two very different synthetic groups that both happen to match the cluster mean. The penalized estimator takes things further by adding distance-based penalties to match the cluster well. While we can use outcomes here (as my current code does), there is no reason why we could not use a different  distance metric to favor units that are literally closer in terms of geography/other factors:
...
Finally, the unit-level estimator introduces the most granular setup. It not only enforces closeness to cluster means but also requires that each treated neighborhood has a tight match with its synthetic control:
...
The experimental SC framework produces a recommendation: given the priorities and constraints, here is the set of neighborhoods that should be treated, and here is the synthetic control to compare them against.  

The government or client then takes these recommendations and implements the policy, working with hotel associations, local communities, or other relevant stakeholders. Ethical and political considerations are respected because the framework allows analysts to control for fairness and logistical feasibility in advance.  

After the treatment is rolled out and enough time has passed, analysts can return to the synthetic control weights and use them to estimate and conduct inference about the treatment effects. The same weights that informed the design now form the backbone of the causal comparison.
...
This approach bridges the gap between theory and practice. It answers the question that truly matters for applied practitioners: given our goals, constraints, and the characteristics of our market, which neighborhoods should we treat, and why?  

## Inference

We now evaluate the treatment effects implied by the experimental synthetic control design. As we have been discussing, the design phase determines which neighborhoods are treated versus control and how clusters are formed; the inference phase estimates the treatment effects and quantifies their uncertainty. Specifically, we (1) compute unit-level differences between synthetic treated and synthetic control outcomes, (2) aggregate these differences to obtain cluster-weighted and overall effects, and (3) use blank pre-treatment periods to quantify uncertainty via split-conformal confidence intervals and permutation tests.

The full pre-treatment period is $\mathcal{T}_0 = \{1, \dots, T_0\}$, with $T_0 = 128$ periods in the Curaçao example. This is divided into the fitting period $\mathcal{T}_1 = \{1, \dots, T_0 - |\mathcal{B}|\}$, used to compute the synthetic control weights ($\mathbf{w}_j$, $\mathbf{v}_j$) and treatment assignments ($z_j$), and the blank period $\mathcal{B} = \{T_0 - |\mathcal{B}| + 1, \dots, T_0\}$, or the final group of periods that are withheld for placebo tests, with $|\mathcal{B}| = 28$ periods. The post-treatment period is $\mathcal{T}_1 = \{T_0 + 1, \dots, T_0 + 28\}$, also covering 28 periods. Thus, $\mathcal{T}_0 = \mathcal{T}_0 \cup \mathcal{B}$.
...
$$
y_{j \in \mathcal{N}_1t} = \sum_{j \in I_k,z_{j} = 1}w_{j} y_{jt},
$$

and the synthetic control outcome is:

$$
y_{j \in \mathcal{N}_0t} = \sum_{j \in I_k,z_{j} = 0}v_{j}y_{jt}.
$$

The unit-level treatment effect for neighborhood $j$ at post-treatment time $t \in \mathcal{T}_1$ is:

$$
\hat{\tau}_{j \in \mathcal{N}_1t} = y_{j \in \mathcal{N}_1t} - y_{j \in \mathcal{N}_0t}, \quad j \in \mathcal{J}, \, t \in \mathcal{T}_1.
$$
...
$$
\hat{\tau}_{k t} = \sum_{j \in I_k,z_j = 1} f_j \hat{\tau}_{j \in \mathcal{N}_1t}, \quad k \in \mathcal{K}, \, t \in \mathcal{T}_1.
$$

The overall effect across all clusters, what I call the global effect at time $t \in \mathcal{T}_1$, is:

$$
\hat{\tau}_{t} = \sum_{k=1}^K \hat{\tau}_{k t} = \sum_{j \in \mathcal{J} : z_j = 1} f_j \hat{\tau}_{j \in \mathcal{N}_1t}, \quad t \in \mathcal{T}_1.
$$

Here, $\sum_{j \in \mathcal{J}} f_j = 1$ weights all clusters (at present my code does this uniformly but presumably we may do this too using other weights, I'll program that at some point).

### Placebo Effects

Placebo effects provide a reference distribution for treatment effects under the null hypothesis of no treatment, using the blank pre-treatment periods $\mathcal{B}$. We compute placebo effects using the same synthetic control weights ($\mathbf{w}_j$, $\mathbf{v}_j$) and treatment assignments ($z_j$) derived from the fitting period $\mathcal{T}_0 \setminus \mathcal{B}$ only. For neighborhood $j$ at time $t \in \mathcal{B}$, the unit-level placebo effect is:

$$
\hat{\tau}_{j \in \mathcal{N}_1t}^{\mathcal{B}} = y_{j \in \mathcal{N}_1t} - y_{j \in \mathcal{N}_0t}, \quad j \in \mathcal{J}, \, t \in \mathcal{B},
$$

where $y_{j \in \mathcal{N}_1t}$ and $y_{j \in \mathcal{N}_0t}$ are defined as above, using the observed outcomes in $\mathcal{B}$ and the weights from $\mathcal{T}_0$. By this, the cluster-level placebo effects (which we are concerned with) are:

$$
\hat{\tau}_{k t}^{\mathcal{B}} = \sum_{j \in I_k,z_j = 1} f_j \hat{\tau}_{j \in \mathcal{N}_1t}^{\mathcal{B}}, \quad k \in \mathcal{K}, \, t \in \mathcal{B}.
$$

The overall global placebo effect is:
...
These placebo effects quantify the variability of synthetic differences in the absence of treatment, serving as a benchmark for statistical inference. In other words, we know that the treatment was not enacted in the blank periods, so at use them as a substitute for the normal confidence intervals we usually get from regression.

### Split-Conformal Confidence Intervals

To quantify uncertainty, we construct split-conformal confidence intervals using the placebo effect distribution. These intervals ensure that the true effect lies within the bounds with probability $1-\alpha$ in repeated samples, based on the variability observed in $\mathcal{B}$. For the overall effect at $t \in \mathcal{T}_1$, we compute the $(1-\alpha)$-quantile of the absolute overall placebo effects:
...
$$
\mathrm{CI}_{t} = \big[\hat{\tau}_{t} - q_{1-\alpha}, \, \hat{\tau}_{t} + q_{1-\alpha}\big], \quad t \in \mathcal{T}_1.
$$
...
$$
\mathrm{CI}_{k t} = \big[\hat{\tau}_{k t} - q_{k,1-\alpha}, \, \hat{\tau}_{k t} + q_{k,1-\alpha}\big], \quad k \in \mathcal{K}, \, t \in \mathcal{T}_1.
$$
...
$$
S_{\text{obs}} = \frac{1}{|\mathcal{T}_1|} \sum_{t \in \mathcal{T}_1} \big|\hat{\tau}_{t}\big|.
$$

We form a permutation distribution by pooling the placebo effects $\{\hat{\tau}_{t}^{\mathcal{B}} : t \in \mathcal{B}\}$ and post-treatment effects $\{\hat{\tau}_{t} : t \in ## Inference

We now evaluate the treatment effects implied by the experimental synthetic control design. As we have been discussing, the design phase determines which neighborhoods are treated versus control and how clusters are formed; the inference phase estimates the treatment effects and quantifies their uncertainty. Specifically, we (1) compute unit-level differences between synthetic treated and synthetic control outcomes, (2) aggregate these differences to obtain cluster-weighted and overall effects, and (3) use blank pre-treatment periods to quantify uncertainty via split-conformal confidence intervals and permutation tests.

The full pre-treatment period is $\mathcal{T}_0 = \{1, \dots, T_0\}$, with $T_0 = 128$ periods in the Curaçao example. This is divided into the fitting period $\mathcal{T}_1 = \{1, \dots, T_0 - |\mathcal{B}|\}$, used to compute the synthetic control weights $(\mathbf{w}_j, \mathbf{v}_j)$ and treatment assignments $(z_j)$, and the blank period $\mathcal{B} = \{T_0 - |\mathcal{B}| + 1, \dots, T_0\}$, the final group of periods that are withheld for placebo tests, with $|\mathcal{B}| = 28$. Thus, $\mathcal{T}_0 = \mathcal{T}_1 \cup \mathcal{B}$. 

The post-treatment period is $\mathcal{T}_2 = \{T_0 + 1, \dots, T_0 + 28\}$, also covering 28 periods.

We estimate treatment effects by comparing synthetic treated and control outcomes. For neighborhood $j \in \mathcal{J}$, the synthetic treated outcome at time $t$ is constructed using the weights $\mathbf{w}_j$ from the design phase:

$$
y_{j \in \mathcal{N}_1t} = \sum_{j \in I_k, z_j = 1} w_j y_{jt},
$$

and the synthetic control outcome is:

$$
y_{j \in \mathcal{N}_0t} = \sum_{j \in I_k, z_j = 0} v_j y_{jt}.
$$

The unit-level treatment effect for neighborhood $j$ at post-treatment time $t \in \mathcal{T}_2$ is:
...
The overall effect across all clusters, what I call the global effect at time $t \in \mathcal{T}_2$, is:

$$
\hat{\tau}_{t} = \sum_{k=1}^K \hat{\tau}_{k t} = \sum_{j \in \mathcal{J} : z_j = 1} f_j \hat{\tau}_{j \in \mathcal{N}_1t}, \quad t \in \mathcal{T}_2.
$$

Here, $\sum_{j \in \mathcal{J}} f_j = 1$ weights all clusters (at present my code does this uniformly, but presumably we may use other weight schemes as well).

### Placebo Effects

Placebo effects provide a reference distribution for treatment effects under the null hypothesis of no treatment, using the blank pre-treatment periods $\mathcal{B}$. We compute placebo effects using the same synthetic control weights $(\mathbf{w}_j, \mathbf{v}_j)$ and treatment assignments $(z_j)$ derived from the fitting period $\mathcal{T}_1$ only. For neighborhood $j$ at time $t \in \mathcal{B}$, the unit-level placebo effect is:

$$
\hat{\tau}_{j \in \mathcal{N}_1t}^{\mathcal{B}} = y_{j \in \mathcal{N}_1t} - y_{j \in \mathcal{N}_0t}, \quad j \in \mathcal{J}, \, t \in \mathcal{B},
$$

where $y_{j \in \mathcal{N}_1t}$ and $y_{j \in \mathcal{N}_0t}$ are defined as above, using the observed outcomes in $\mathcal{B}$ and the weights from $\mathcal{T}_1$. By this, the cluster-level placebo effects are:
