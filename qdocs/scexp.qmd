...
Observed outcomes, like occupancy rates or energy use, are collected in $\mathbf{Y} \in \mathbb{R}^{J \times T_0}$, with $Y_{j,t}$ representing the outcome for neighborhood $j$ at time $t$. Potential outcomes are $Y_{j,t}^I$ if the neighborhood participates in Green Stay and $Y_{j,t}^N$ if not. The cluster-weighted average treatment effect is
...
The optimization problem chooses synthetic treated and control neighborhoods by minimizing a loss function subject to feasibility constraints. At its core, the method ensures that within each cluster of neighborhoods, we construct weighted averages that resemble both the cluster as a whole and each other.
...
To evaluate uncertainty in synthetic control estimates, we use a placebo-based inference procedure. The key idea is that we can treat the blank pre-treatment periods (periods that were not used for fitting) as if they were "pseudo post-treatment" periods. This lets us simulate the kind of variation we might expect under the null of no effect.

Formally, let

$$
\tau_t = Y^{\text{treat}}_t - Y^{\text{control}}_t
$$

denote the estimated treatment effect in post-treatment period $t$, obtained from the difference between the synthetic treated group and the synthetic control group.

During the blank periods, we compute analogous differences

$$
u_s = Y^{\text{treat}}_s - Y^{\text{control}}_s, \quad s \in \{T_0 - B + 1, \dots, T_0\},
$$

where $B$ is the number of blank periods and $T_0$ is the intervention start. Since the intervention has not occurred yet, these placebo gaps $u_s$ provide an empirical distribution of “false effects.”

To construct confidence intervals, we compare each estimated effect $\tau_t$ against the distribution of placebo gaps. Specifically, let

$$
q_{1-\alpha} = \text{Quantile}_{1-\alpha}(|u_s|),
$$

then the $(1-\alpha)$-confidence interval is given by

$$
\Big[\, \tau_t - q_{1-\alpha} \cdot \sigma, \; \tau_t + q_{1-\alpha} \cdot \sigma \,\Big],
$$

where $\sigma$ rescales the placebo distribution by the ratio of the post-treatment RMSPE to the pre-treatment RMSPE.

P-values are computed as the fraction of placebo gaps at least as large in absolute value as the observed effect:

$$
p_t = \frac{1}{B} \sum_{s=1}^B \mathbf{1}\{ |u_s| \geq |\tau_t|\}.
$$

This procedure applies both at the global level (aggregating across clusters) and cluster-by-cluster, with cluster-specific scaling by pre-fit RMSE.

The resulting inference is non-parametric, relying on observed pre-treatment variation rather than asymptotic approximations. It provides a principled way to quantify uncertainty around synthetic control estimates while respecting the data structure.
