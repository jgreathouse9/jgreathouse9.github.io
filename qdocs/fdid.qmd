...
Let units be indexed by $i$ and time by $t$. Let $\mathcal{N}$ be the set of all units and $\mathcal{U}$ be some subset of $\mathcal{N}$. For our purposes, $i = 0$ denotes the treated unit, and $i \in \mathcal{N}$ represents the control group. Parallel trends for Difference-in-Differences (DID) can be formalized as:
