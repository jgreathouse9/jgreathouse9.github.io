...
I normalized all the metrics to 100 at August 17, 2023, a few days before the treatment [occured](https://www.gq.co.za/culture/entertainment/a-timeline-tracking-tylas-water-from-the-streets-of-johannesburg-to-global-domination-657bad00-e7dc-4823-aea2-9529b335c2d1) (Tyla performing in Kigali, Rwanda). We can see that Tyla generally had less relative popularity for the majority of the pretreatment time series compared to the donors. We can see both her metrics balloon in the post-intervention period. We can also see quite clearly that parallel trends with respect to the donor pool likely does not hold; the trend of the average of controls is quite flat relatve to Tyla's. When we estimate the $\ell_2$ relaxer, we get the following plot (note the red line is the counterfactual Tyla, or how her Playlist Reach and Playlist Count would look absent her going viral) :
...
Here is the same plot with the Apple data
...
|:--------------------|:----------------------------------------------------------------|
| ATT                 | 407.611                                                         |
| Standard Error      | 12.817643066353503                                              |
| t-stat              | 712.5077303234048                                               |
| Confidence Interval | (382.428,432.79) |
| RMSE (T0)           | 5.585                                                           |
| R-Squared           | 0.971                                                           |


This should surprise nobody, and the practical results speak for themselves. Going viral clearly contributed a lot to Tyla's success across both platforms, as the estimated counterfactuals spell out very plainly. As we know from the above, every artist in the donor pool contributes to Tyla's synthetic control. The method clearly outperforms the Difference-in-Differneces method, including Forward DID (another implementation from ``mlsynth``). This method can also be used with multiple treatd units, but I haven't written this just yet. There are a few caveats to this, though:

- For one, these aren't raw Spotify or Apple data, so there's certainly measurement error in these metrics. For some reason, Songstats aggregates their data funny, only having, for example, Playlist Reach data for [Rihanna](https://songstats.com/artist/p3uwqx2a/rihanna?source=spotify) going back to 2023.
- Secondly, the donor pool is kind of small. Songstats latent API that I used to collect these metrics isn't really built to grab many artists en masse; I'm sure it's possible with a little Selenium work (all of it rests on collecting the unique artist ID's, for example Tyla's is: 7ln14gwj), but I was too lazy to do that. So in theory, it would be possible to add many more artists to this dataset to see how the estimator plays when there are truly 4 or 500 artists to work with, in some sense.
