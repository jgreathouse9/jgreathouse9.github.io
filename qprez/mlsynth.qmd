...
- Whether we're in business or government, maximizing value for business services and crafting effective public policy depends on knowing whether policies, promotions, new web features, or other interventions affect some metric that we care about in an appreciable, real way.

- Researchers commonly use Difference-in-Differences designs and Synthetic Control methods (DID and SCM) to measure causal impacts./ Both are widely available in statistical softwares like R, Stata, and Python.
...
- SCM struggles with high-dimensional datasets. Often, smaller donor pools are preferable to very large ones to mitigate overfitting. But picking a good donor pool can be challenging when there are very many controls. Also, SCM solutions are often sparse, given the classical formulation of the problem's weights are nonnegative and add to 1.
