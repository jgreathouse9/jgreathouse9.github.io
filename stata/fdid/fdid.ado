...
scalar omegahatdid = sqrt(((scalar(`t2')/scalar(`t1'))*r(Var))+r(Var))
...
scalar CILB= scalar(ATT) - ((invnormal(0.975) * scalar(omegahatdid))/sqrt(`t2'))

scalar CIUB= scalar(ATT) + ((invnormal(0.975) * scalar(omegahatdid))/sqrt(`t2'))


loc rmseround: di %9.5f `RMSE'

qui mkmat *, mat(series)
scalar tstat = abs(scalar(ATT)/scalar(omegahatdid))

matrix my_matrix = (scalar(ATT), scalar(omegahatdid), scalar(tstat), scalar(CILB), scalar(CIUB), scalar(r2), `rmseround')
...
di as text %12s abbrev("`treatment'",12) " {c |} " as result %9.3f scalar(ATT) " "%9.3f scalar(tstat) "    " %9.4f scalar(omegahatdid) "    " %9.4f scalar(CILB) "   " %9.4f scalar(CIUB)  %9.3f scalar(p_value)     ""
