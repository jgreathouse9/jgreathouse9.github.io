...
I begin with the Spotify data. When we estimate $\ell_2$-PDA, comparing Tyla to her 59 controls, we get the following plot. The red line is the counterfactual Tyla, how her Playlist Reach would look absent her going viral, and the shaded red band around it is a 95% prediction interval for that counterfactual on each post-treatment day. The band is new to ``mlsynth`` and it is what the [last section](#prediction-intervals-when-did-water-actually-hit) is about; for now, read it as the range of untreated outcomes the model considers plausible on that day. Where the black line leaves the band, the effect on that day is significant on its own.
...
def fit_pda(df, outcome_col):
    """Fit the l2 relaxation with the Jiang et al. (2025) intervals attached."""
    config = {
        "df": df,
        "treat": "Water",
        "time": "Date",
    ...
    return results, results.fits["l2"]


def banded_plot(df, outcome_col, fit, title, ylabel):
    """Tyla against her counterfactual, with the counterfactual's own band."""
    unit = df[df["Artist"] == "Tyla"].sort_values("Date")
    days = pd.to_datetime(unit["Date"].to_numpy())
    observed = unit[outcome_col].to_numpy(dtype=float)
    counterfactual = np.asarray(fit.counterfactual, dtype=float)
    post = unit["Water"].to_numpy() == 1
    band = fit.prediction_intervals["counterfactual"]
    lower, upper = np.asarray(band["eq_lower"]), np.asarray(band["eq_upper"])

    fig, ax = plt.subplots(figsize=(11, 5))
    ax.fill_between(days[post], lower, upper, color="red", alpha=0.20, linewidth=0,
                    label="95% prediction interval")
    ax.plot(days, counterfactual, color="red", linewidth=1.6, label="Counterfactual Tyla")
    ax.plot(days, observed, color="black", linewidth=1.6, label="Tyla")
    ax.axvline(days[post][0], color="blue", linestyle="--", linewidth=0.9, label="Water")

    above = np.flatnonzero(observed[post] > upper)
    if above.size:
        i = int(above[0])
        ax.plot(days[post][i], observed[post][i], marker="o", markersize=6,
                color="black", zorder=5, label="_nolegend_")
        ax.annotate(f"leaves the band\n{pd.Timestamp(days[post][i]).date()}",
                    xy=(days[post][i], observed[post][i]), xytext=(-24, 88),
...
def summarize(results, fit, df, outcome_col):
    """Every number the prose quotes, so none of them is typed by hand."""
    eff = fit.prediction_intervals["effect"]
    lower, upper = np.asarray(eff["eq_lower"]), np.asarray(eff["eq_upper"])
    point = np.asarray(eff["point"])
    dates = pd.to_datetime(np.sort(df.loc[df["Water"] == 1, "Date"].unique()))
    att, se = results.effects.att, results.effects.att_std_err

    def first(mask, default=None):
        hit = np.flatnonzero(mask)
        return int(hit[0]) if hit.size else default
    ...
    return {
        ...
        "se": f"{se:,.1f}",
        "t": f"{att / se:,.1f}",
        "ci": f"[{results.inference.ci_lower:,.0f}, {results.inference.ci_upper:,.0f}]",
        "rmse": f"{results.fit_diagnostics.rmse_pre:,.1f}",
        "p": ("below 0.001" if results.inference.p_value < 0.001
              else f"{results.inference.p_value:.3f}"),
        "studentization": fit.prediction_intervals["studentization"],
        "onset": "never" if onset is None else f"{dates[onset]:%B %-d, %Y}",
        "onset_t": "-" if onset is None else f"{onset + 1}",
        "peak": f"{point.max():,.0f}",
        "peak_day": f"{dates[int(point.argmax())]:%B %-d, %Y}",
        "spanning": f"{int(((lower <= 0) & (upper >= 0)).sum())}",
        "n_post": f"{point.size}",
        "width_first": f"{upper[0] - lower[0]:,.0f}",
        "width_last": f"{upper[-1] - lower[-1]:,.0f}",
        "n_below": f"{first(upper >= 0, point.size)}",
        "n_pre": f"{int((df[df.Artist == 'Tyla'].Water == 0).sum()):,}",
        "n_donors": f"{df.Artist.nunique() - 1}",
    }


spotify_results, spotify_fit = fit_pda(spotify_df, outcome)
banded_plot(spotify_df, outcome, spotify_fit,
            "Spotify: Tyla's Playlist Reach against her counterfactual",
            "Playlist reach (100 = August 17, 2023)")
sp = summarize(spotify_results, spotify_fit, spotify_df, outcome)
```

Over the `{python} sp['n_post']` post-treatment days the average effect is `{python} sp['att']` index points of playlist reach, with a standard error of `{python} sp['se']`, a $t$ of `{python} sp['t']`, a 95% confidence interval of `{python} sp['ci']`, and a $p$-value `{python} sp['p']`. The single largest daily effect is `{python} sp['peak']` on `{python} sp['peak_day']`. The pre-treatment fit that all of this rests on has a root mean squared error of `{python} sp['rmse']` against a series normalized to 100, so the counterfactual tracks the pre-period closely.
...
apple_results, apple_fit = fit_pda(apple_df, appleoutcome)
banded_plot(apple_df, appleoutcome, apple_fit,
            "Apple Music: Tyla's Playlist Count against her counterfactual",
            "Playlist count (100 = August 17, 2023)")
ap = summarize(apple_results, apple_fit, apple_df, appleoutcome)
```


Here the average effect over the `{python} ap['n_post']` post-treatment days is `{python} ap['att']` index points of playlist count, standard error `{python} ap['se']`, $t$ of `{python} ap['t']`, 95% confidence interval `{python} ap['ci']`, $p$-value `{python} ap['p']`, peaking at `{python} ap['peak']` on `{python} ap['peak_day']`, on a pre-treatment fit with a root mean squared error of `{python} ap['rmse']`. The Apple pre-period is much longer than the Spotify one, `{python} ap['n_pre']` days against `{python} sp['n_pre']`, and the donor pools differ too: `{python} ap['n_donors']` artists here against `{python} sp['n_donors']` there.
...
``mlsynth`` now attaches the bootstrap prediction intervals of Jiang, Li, Shen and Zhou (2025) to every fitted PDA variant. Set ``prediction_intervals=True`` and you get a band around each period's counterfactual and around each period's effect; the two are the same object, since the effect is the observed outcome minus the counterfactual and the observed outcome is not random. The shaded bands in the two plots above are the counterfactual version, which is why the day the black line leaves the band is also the day the effect interval clears zero.
...
Three things fall out of the bands, and none of them is visible in the ATT.

The first is the onset date. On Spotify the black line leaves the band on `{python} sp['onset']`, day `{python} sp['onset_t']` of the post-period; on Apple Music it is `{python} ap['onset']`, day `{python} ap['onset_t']`. These are two separate fits with different donor pools, different outcome metrics, and pre-periods of very different lengths, `{python} sp['n_pre']` days against `{python} ap['n_pre']`. That they agree to the day is the sort of corroboration a single averaged number cannot give you. It also lines up with the story: the Kigali performance is the spark, but the effect on playlist behavior takes a couple of weeks to show up above the noise. Before that day the two lines are tangled together, and the band is where you can see why: on Spotify `{python} sp['spanning']` of the `{python} sp['n_post']` post-treatment days have an interval that still contains zero, and on Apple `{python} ap['spanning']` of `{python} ap['n_post']`.

The second is that the *shape* of the uncertainty differs sharply across platforms, which I did not expect. Apple's band is essentially a constant width, opening at `{python} ap['width_first']` index points and ending at `{python} ap['width_last']`. Spotify's opens at `{python} sp['width_first']` and ends at `{python} sp['width_last']`, so it fans out as the post-period runs on. The counterfactual Tyla on Spotify becomes progressively harder to predict the further we extrapolate from the pre-period, while the Apple counterfactual stays about as predictable in June 2024 as it was in August 2023. If you were using this to make a decision, that difference matters more than the ATT does: the same headline effect is a much shakier claim on one platform than the other, and only the per-period bands say so.

The third is an honest wrinkle. On Spotify the first `{python} sp['n_below']` days of the post-period sit entirely *below* the band, a significantly negative effect, before the line crosses over and stays above. I would not read much into it, but I would not hide it either. It is the kind of thing that an average effect of `{python} sp['att']` papers over completely.

One implementation note: both fits here report `{python} sp['studentization']` studentization, meaning the post-selection OLS HAC variance was available. The $\ell_2$ relaxation falls back to a $\sigma^2$-only studentization when the controls outnumber the pre-periods, which is the genuinely dense regime this estimator is built for. That is not the case here (`{python} sp['n_donors']` donors against `{python} sp['n_pre']` pre-periods on Spotify), so we get the sandwich. If you scale this up to the "4 or 500 artists" I complain about not having below, expect the fallback instead.
