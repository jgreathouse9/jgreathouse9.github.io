...
$$  
The key challenge of causal inference is that for each unit, we only observe the factual outcome, expressed as:  
$$
y_{it} = y_{it}^1 d_{it} + \left(1 - d_{it}\right)y_{it}^0,
$$
where $y_{it}^1$ is the potential outcome under treatment, $y_{it}^0$ is the counterfactual (or potential) outcome under no treatment, and $d_{it} \in \{0, 1\}$ is an indicator for treatment (where $d_{it} = 1$ if treated, $d_{it} = 0$ if untreated). The objective is to estimate the counterfactual outcomes $y_{1t}^0$ for the treated unit during the post-treatment period, $t \in \mathcal{T}_2$. SCMs and panel data approaches are weighting based estimators, where some weight vector $\mathbf{w}$ is assigned across the $N_0$ control units
