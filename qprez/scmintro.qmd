...
- The weights representing each person’s contribution to the average is a column vector:
...
- Then the average is a proper matrix multiplication:
...
This matches our scalar calculation, now expressed with vectors.

---

### Matrices

- Let’s extend this to multiple time periods. Suppose today you have 12 apples and tomorrow 30, and your friend has 18 today and 22 tomorrow. We can organize this into a matrix, where each row is a time period and each column is a person:
...
- Using the same weights as before, the average per time period is:
...
Instead of treating Bill as equal to you two, we can give each person a "weight" $w_i$ to decide how much they count in the average:  

$$
\bar{x}_{\text{weighted}} = \sum_{i=1}^{n} w_i x_i, \quad \sum_{i=1}^{n} w_i = 1
$$

Now Bill can get a tiny weight, and you and your friend larger weights, giving a more reasonable "average bar income."  
...
## Familiar Averaging Estimators

- The issue with the above is that it's kind of arbitrary. There's no reason that the weights for us both need to be 0.5. To use the idea of a weighted average for counterfactual prediction, we'll need to formalize this a little more.
...
- Collecting data on their GDPs over the pre and post terrorism periods, we wish to estimate the counterfactual for the Basque Country as a weighted average of control units. Below we have the Basque Country's GDP, and the GDPs of the 16 other untreated regions.
...
But how do we determine the weights? How do we know which unit should be weighed by how much? One way to do this is to use linear regression.

::: {.panel-tabset}

### Notation

- $\mathbf{y}_1 \in \mathbb{R}^{T}$: vector of outcomes (GDP per capita) for the treated unit (Basque Country) over $T$ time periods  
...
In Stata this would just be `reg y Donor1, Donor2-DonorN if year < 1975`, where the dependent variable is the GDP for the Basque Country and the independent variables are the GDP values of the untreated units. Here, the OLS coefficients are our weights.
...
The average treatment effect on the treated can be expressed as the mean difference between the observed outcomes and the synthetic control predictions over the post-intervention period:
...
As we can see, we couldn't really interpret very much from the weights. While they did in fact produce a counterfactual, it does not seem like a very believable one, and we don't know really which region of Spain mattered most to predicting the counterfactual. What if we could change the weights somehow?
