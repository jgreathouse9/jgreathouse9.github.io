...
https://doi.org/10.48550/arXiv.2203.11576) in settings where we have multiple covariates that can potentially inform our selection of the donor pool. Asit turns out though, when we employ machine learning methods to select the donor pool, using outcome data only, we can get comparable results to the baseline estimates which seem to require multiple covariates in order to properly fit the pre-intervention trajectory. The promise of machine-learning methods in this space is to automate away donor/predictor selection, to some degree, which can attain similar results to the original implementations of the methodology.
