...
- **Connection to Difference-in-Differences (DiD)**  
  - DiD assumes outcomes follow:  
    ...
    $$
    where $\mu_j$ are unobservable unit constant effects and $\lambda_t$ are time constant, unit variant unobserved effects  
...
## [Key Takeaways for Practitioners](https://doi.org/10.48550/arXiv.2203.06279)

- **Low-volatility series**  
  Synthetic controls work best with aggregate, relatively smooth series. Trying to fit a highly volatile outcome can lead to overfitting, especially with short pre-treatment periods.  

- **Extended pre-intervention period**  
  A good synthetic control should closely track the treated unit over a long pre-treatment period. Persistent pre-treatment fit indicates that unobserved confounders are being captured effectively.  

- **Small, relevant donor pool**  
  More donors isn’t always better. Focus on a small set of untreated units that are similar to the treated unit in predictors to reduce overfitting and interpolation biases.  

- **Sparsity aids interpretability**  
  A synthetic control built from a few key donors is easier to interpret and assess.  

- **Plot your treated unit versus the donors (seriously, before you run your model).**  
  Visualizing the treated unit against the donor pool helps identify anomalies and diagnose extreme weights.  

- **Good pre-treatment fit matters**  
  The validity of the synthetic control depends on close pre-intervention fit. A poor fit undermines credibility and can bias treatment effect estimates.  

- **Out-of-sample validation is essential**  
  Check predictive performance using placebos tests. This ensures your synthetic control is capturing meaningful patterns rather than noise.  
