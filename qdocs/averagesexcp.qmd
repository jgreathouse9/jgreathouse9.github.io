...
The point of this post is to show that the arithmetic average is the solution to an optimization problem, and connect it to how we think about effect size existing for causal inference.
...
However, even $\sum_{i=1}^{n} (x_i - \mu)$ is not enough because positive and negative deviations could cancel each other out. To penalize deviations regardless of sign, we square each term, giving the objective function:
...
The inner function is $u(\mu) = x_i - \mu$. From the derivative of a constant, we have:
...
Finalky, summing over all $i$ gives the derivative of the entire objective function:

$$
\frac{\mathrm{d}}{\mathrm{d}\mu} \sum_{i=1}^{n} (x_i - \mu)^2 = \sum_{i=1}^{n} [-2(x_i - \mu)] = -2 \sum_{i=1}^{n} (x_i - \mu),
$$

where we factor out the constant -2 from the summation. As usual, setting this derivative equal to zero gives us the first-order condition we need to find the minimum point:

$$
-2 \sum_{i=1}^{n} (x_i - \mu) = 0.
$$

Now we solve for the decision variable. We begin by dividing both sides by -2. This simplifies the equation to

$$
\sum_{i=1}^{n} (x_i - \mu) = 0.
$$

Next, we may split the sum into two parts to isolate $\mu$:
...
Rearranging the equation to isolate $\mu$, add $n\mu$ to the right hand side. That gives us this result:

$$
\sum_{i=1}^{n} x_i = n \mu.
$$

Finally, we arrive at the arithmetic mean by dividing the right hand side by $n$. This gives us the definition of the arithmetic mean:
...
I think part of the reason for this anxiety around weighted averages with SCM is that the simple arithmetic average of sieve subset of controls is framed as Not a Decision. The arithmetic average weights just are what they are, so much so that many people (I suspect) do not even see it as a decision they are making. Presumably this is also why the uniform weights (as in, say, [Difference-in-Differences](https://arxiv.org/abs/1610.07748)) are never questioned, but the weights from SCM are met with suspicion.
