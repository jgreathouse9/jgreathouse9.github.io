...
Although the current implementation uses uniform weights for simplicity, alternative schemes—such as population- or traffic-weighted averages—could also be applied.

To understand the variability in our estimates and build confidence intervals, we look at placebo effects. These are computed in the blank pre-treatment period $\mathcal{B}$ and capture the variation of synthetic differences when no treatment is applied. The unit-level placebo effect is

$$
\hat{\tau}_{j \in \mathcal{N}_1 t}^{\mathcal{B}} = y_{j \in \mathcal{N}_1 t} - y_{j \in \mathcal{N}_0 t}, \quad t \in \mathcal{B},
$$

and cluster-level and global placebo effects are defined similarly. By examining these distributions, we can gauge the natural fluctuations in the synthetic control differences.

Using the placebo distributions, split-conformal confidence intervals can be constructed. For the global effect, we take the $(1-\alpha)$ quantile of the absolute placebo effects,
...
By sampling permutations from placebo and post-treatment effects, we generate permuted statistics
...
Cluster-level significance tests follow the same logic using cluster-specific effects and placebo distributions. In this way, the combination of synthetic control construction, placebo comparisons, and permutation testing provides a complete and interpretable framework for estimating treatment effects while accounting for uncertainty.
