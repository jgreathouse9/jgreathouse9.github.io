...
Exploiting the fact that summation is a linear operator, it distributes over addition and allows constants to factor out. Since $\mu$ does not at all depend on $i$, it appears identically in every term of the sum. Beyond, this is exactly what we seek: one single value that is as close as possible to all the data points simultaneously
...
Now we substitute this result into the second term on the LHS. That gives us this result:
...
One way to think about this intuitively is to imagine the data points as a small cloud in $n$-dimensional space. The subspace $C$ is a line running through the origin, and the projection of the data vector onto this line is the point on the line that comes closest to the entire cloud. You can think of it as the line that best "fits" the cloud in the sense of minimizing Euclidean distance from each point to the line. In one dimension, this line is trivial, as it is just the line of constant vectors.

A basic fact from linear algebra is that the closest point in a subspace to a given vector is obtained by orthogonal projection. This means that at the optimal value $\hat{\mu}$, the residual vector

$$
\mathbf{x} - \hat{\mu} \mathbf{1}
$$

is perpendicular to the subspace $C$, which is spanned by $\mathbf{1}$. Orthogonality can be expressed as

$$
\mathbf{1}^\top (\mathbf{x} - \hat{\mu} \mathbf{1}) = 0.
$$

Expanding this gives

$$
\mathbf{1}^\top \mathbf{x} - \hat{\mu} \mathbf{1}^\top \mathbf{1} = 0.
$$
...
Thus the arithmetic mean is the projection of the data vector onto the subspace of constant vectors. The derivative-based condition from the calculus approach is equivalent to this orthogonality condition expressed componentwise. Seen this way, the arithmetic mean is simply the closest constant vector to the data, and the geometric derivation highlights the same principle that underlies ordinary least squares and synthetic control methods: choosing weights to minimize squared distance.
