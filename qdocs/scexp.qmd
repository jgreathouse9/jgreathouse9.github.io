...
# Notation and Econometric Methodology
...
config = {"df": df,
          ...
          "m_max": 3,
          "cluster": "Region",
          "program_type": "MIQP", "blank_periods": 28, "inference": True, "display_graph": True, "lambda1": 0.02, "lambda2": 0.02}
...
config = {"df": df,
          ...
          "m_max": 5,
          "cluster": "Region",
          "program_type": "MIQP", "blank_periods": 28, "inference": True, "display_graph": True}

design = MAREX(config).fit()

```

In the context of the Curaçao Green Stay example, the synthetic control analysis was extended to a single-cluster setup, treating all units as part of one cluster to estimate treatment effects using simulated data and Mixed Integer Quadratic Programming. Below is a Quarto document summarizing the results for this one-cluster setup, presented narratively with tables for weight distributions, as requested. The pre-treatment RMSE is 0.449, which, given outcome values in the 40s, represents a very small error and a reasonable fit for analysis. The summary integrates these results with the previous two-cluster findings for a cohesive overview.text# Curaçao Green Stay: Synthetic Control Analysis Summary

This document presents the results of a synthetic control analysis applied to the Curaçao Green Stay example, using simulated data to estimate treatment effects via Mixed Integer Quadratic Programming. The analysis constructs synthetic treated and control units, with pre-treatment fit assessed by root mean squared error (RMSE). Two setups are considered: a two-cluster setup (Non-Willemstad and Willemstad) and a one-cluster setup treating all units together. All configurations yield acceptable fits, with the two-cluster setup showing varying precision and the one-cluster setup providing a reasonable fit relative to the outcome scale.

## Two-Cluster Setup: Non-Willemstad (Cluster 0)

For the Non-Willemstad cluster, the pre-treatment RMSE is 0.215, reflecting an excellent fit that ensures a highly accurate synthetic control. The control group draws from the following units:
...
The treated group comprises these units:
...
This low RMSE of 0.215 underscores a robust synthetic control, with Westpunt exerting the strongest influence on the control group at 53.3% and Sint Willibrordus leading the treated group at 38.2%. The balanced weights and excellent fit suggest highly reliable treatment effect estimates for non-Willemstad areas.

## Two-Cluster Setup: Willemstad (Cluster 1)

For the Willemstad cluster, the pre-treatment RMSE is 0.413, indicating a fine fit, though less precise than Cluster 0, yet still suitable for analysis. The control group is constructed from these units:
...
The treated group includes the following units:
...
With an RMSE of 0.413, the fit is reasonable, with Piscadera Bay dominating the control group at 60.5% and Otrobanda leading the treated group at 41.5%. The higher RMSE suggests slightly less confidence in treatment effect estimates for Willemstad, but the fit remains acceptable for analysis.

## One-Cluster Setup: All Units
