...
Exploiting the fact that summation is a linear operator, it distributes over addition and allows constants to factor out. This also fits with intuition, $\mu$ is the same value for every term in the sum and does not depend on $i$. This is exactly what we seek: **one single value** that is as close as possible to all the data points simultaneously. Because $\mu$ is a constant with respect to the indexer $i$, the second sum is simply $\mu$ added to itself $n$ times:
...
Now we substitute this result to the second term on the LHS. That gives us this result:
