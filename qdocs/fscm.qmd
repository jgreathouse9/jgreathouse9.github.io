...
The updated donor set is:

$$
\mathcal{S}_K = \mathcal{S}_{K-1} \cup \{j^*\}.
$$

Finally, the donor pool is chosen as the set that minimizes MSE:

$$
\mathcal{S}^* = \underset{\mathcal{S}_i \in \{\mathcal{S}_1, \mathcal{S}_2, \dots, \mathcal{S}_K\}}{\operatorname*{argmin}} \ MSE(\mathcal{S}_i).
$$

Note that not all donors in $\mathcal{S}^*$ necessarily receive positive weight in the final synthetic control solution.
