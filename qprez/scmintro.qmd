...
line_hatY, = ax.plot(time, w_start[0]*Y0[:,0] + w_start[1]*Y0[:,1], 
                     label="Convex Basque", color="#7DF9FF", linestyle="--", linewidth=2)
...
def update(frame):
    ...
    if frame == steps - 1:
        ...
        ax.set_title(
            f"Final SCM: w_Cataluna={w_curr[0]:.3f}, w_Madrid={w_curr[1]:.3f}, "
            f"Pre-treatment RMSE={rmse_pre:.4f}"
        )
    else:
        ax.set_title(
            f"w_Cataluna={w_curr[0]:.3f}, w_Madrid={w_curr[1]:.3f}, "
            f"Pre-treatment RMSE={rmse_pre:.4f}"
        )
