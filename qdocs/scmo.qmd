...
When I first read about this apporach, I was confused as to what was going on, until I considered them as the solution to a system of multiple equations. To have a better sense of what's going on, we typically consider our outcomes in data science or econoemtrics to be generated by some process that we never see. A common assumption is that they are generated by a [low-rank model](https://jmlr.org/papers/volume19/17-777/17-777.pdf), which essentially formulates our outcomes as a byproduct of the interaction between unit specific fixed effects that are idiosyncratic to each unit (that are not expected to change over time) and time effects that are common across all units of interest. Well, more than one outcome exists in the world that may plausibly have a similar factor structure to the metric that we do care about.

Suppose we are Airbnb, and we wish to see the causal effect of the introduction of [Airbnb Experiences](https://news.airbnb.com/airbnb-partners-with-art-basel-miami-beach/) on [Gross Booking Value](https://www.lodgify.com/encyclopedia/gross-booking-revenue/) (GBV), a metric which is defined as ''the total revenue generated by room or property rentals before any costs or expenses are subtracted''. Well, in theory, Airbnb Expereinces may make the use of the Airbnb more attractive to tourists, since now tourists no longer need use Viator or other booking services to find stuff to do locally. In other words, it makes it an attraction to that city specifically, and we may see an increase in GBV as a result. Well, all sorts of things may be related to this metric, such as local hotel prices, preexisting level of tourist arrivals, temperature, and other relevant metrics.

When we include these in our SCM, we now allow the model to better capture the latent factors that influence the main outcome, thereby improving the match of the synthetic control. The optimization framework effectively allows us to match on both the main outcome and the auxiliary factors, capturing the factor loadings that generate the observed data. Thus, stacking the outcomes together or including an intercept term aligns the model with the idea that multiple systems of equations, representing different outcomes or contributing factors, can provide a better counterfactual prediction than simply using one outcome.

## Model Averaging via Convex Combination of Counterfactuals


Sun, Ben-Michael, and Feller also advocate to model average these together. This is easily implemented as well, arguably even easier than before. Suppose we are given two distinct estimators of the counterfactual outcome for the treated unit. On one hand, we have $\hat{\mathbf{y}}_1^{(1), \text{pre}} \in \mathbb{R}^{T_0}$ which denotes the pre-treatment fit from the concatenated model, and the analogous $\hat{\mathbf{y}}_1^{(2), \text{pre}} \in \mathbb{R}^{T_0}$ which is the same for the demeaned model. As beefore, we have the treated unit’s actual pre-treatment trajectory. With this in mind, let $\hat{\mathbf{Y}}^{\text{pre}} \in \mathbb{R}^{T_0 \times 2}$ denote the matrix stacking the two pre-treatment fits together as columns:

$$
\hat{\mathbf{Y}}^{\text{pre}} = \begin{bmatrix}
\hat{\mathbf{y}}_1^{(1), \text{pre}} & \hat{\mathbf{y}}_1^{(2), \text{pre}}
\end{bmatrix}.
$$

We then define the model-averaged pre-treatment fit as a convex combination of these two prediction vectors:

$$
\hat{\mathbf{y}}_1^{\text{MA, pre}}(\boldsymbol{\lambda}) = \hat{\mathbf{Y}}^{\text{pre}} \boldsymbol{\lambda}, \quad \text{where } \boldsymbol{\lambda} \in \Delta^1,
$$

The optimal $\boldsymbol{\lambda}^\ast$ is selected by minimizing the mean squared error between the treated unit’s true pre-treatment outcomes and the model-averaged fit:

$$
\boldsymbol{\lambda}^\ast = \underset{\boldsymbol{\lambda} \in \Delta^1}{\operatorname{argmin}} \left\| \mathbf{y}_1^{\text{pre}} - \hat{\mathbf{Y}}^{\text{pre}} \boldsymbol{\lambda} \right\|_F^2
$$

where lambda also lives on the simplex 

$$
\Delta^{2} = \left\{ \boldsymbol{\lambda} \in \mathbb{R}_{\geq 0} \: \forall \: i, \| \boldsymbol{\lambda} \| = 1 \right\}.
$$

Note that the idea of the convex hull does not go anywhere, and it menas the same thing here as it does in normal SCM: the model averaged predictions cannot be greater than any outcome from any of the counterfactuals, and it cannot be lower than the lowest outcome estimated by either of the counterfactuals, either. Once the optimal $\boldsymbol{\lambda}^\ast$ is found, the post-treatment counterfactual is obtained analogously via:

$$
\hat{\mathbf{Y}}^{\text{post}} = \begin{bmatrix}
\hat{\mathbf{y}}_1^{(1), \text{post}} & \hat{\mathbf{y}}_1^{(2), \text{post}}
\end{bmatrix},
\quad
\hat{\mathbf{y}}_1^{\text{MA, post}} = \hat{\mathbf{Y}}^{\text{post}} \boldsymbol{\lambda}^\ast.
$$
...
# Run simulation
df = simulate(seed=156, r=2) # r=2 seed 245 gives us maricelo
config = {
    "df": df,
