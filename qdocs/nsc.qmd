...
This term penalizes control units that differ more from the treated unit in pre-treatment trends. The final term is a Tikhonov regularization penalty that shrinks the weight vector toward zero to improve stability. The parameters \$a > 0\$ and \$b > 0\$ control the strength of the \$\ell_1\$ and \$\ell_2\$ penalties, respectively. The constraint \$|\mathbf{w}|_1 = 1\$ ensures that the synthetic control remains on the probability simplex \$\Delta^{N_0 - 1}\$, forming an affine combination of the control units.

## Cross-Validation for Tuning

To select optimal regularization parameters \$(a, b)\$, we perform \$k\$-fold cross-validation using a pseudo-treated framework. Each control unit \$j \in \mathcal{N}_0\$ is treated as if it were the treated unit, and we aim to reconstruct \$\mathbf{y}_j^{\text{pre}}\$ from the remaining controls. For a given pseudo-treated unit \$j\$, let \$\mathcal{N}_0^{(-j)} = \mathcal{N}_0 \setminus {j}\$ and define the donor matrix \$\mathbf{Y}_0^{\text{pre}, (-j)}\$ by removing column \$j\$.

For each fold and pseudo-treated unit, we solve:

$$
\min_{\mathbf{w}^{(j)} \in \mathbb{R}^{N_0 - 1}} \quad \left\| \mathbf{y}_j^{\text{pre}} - \mathbf{Y}_0^{\text{pre}, (-j)} \mathbf{w}^{(j)} \right\|_2^2 + a \sum_{\ell \in \mathcal{N}_0^{(-j)}} \delta_\ell^{(j)} |w_\ell^{(j)}| + b \|\mathbf{w}^{(j)}\|_2^2 \quad \text{s.t.} \quad \|\mathbf{w}^{(j)}\|_1 = 1
$$

where

$$
\delta_\ell^{(j)} = \left\| \mathbf{y}_j^{\text{pre}} - \mathbf{y}_\ell^{\text{pre}} \right\|_2, \quad \forall \ell \in \mathcal{N}_0^{(-j)}
$$
...
$$
\text{MSE}^{(j)} = \frac{1}{T_0} \left\| \mathbf{y}_j^{\text{pre}} - \widehat{\mathbf{y}}_j^{\text{pre}} \right\|_2^2
$$
...
where \$\mathcal{A}\$ and \$\mathcal{B}\$ are grids of candidate values. In practice, we employ `sklearn`’s cross-validation utilities to accelerate computation, as the original approach from Wei (2021) was computationally intensive. This modification performs comparably while dramatically improving runtime.
