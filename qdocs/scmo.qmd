...
You'll need the most recent version which automatically computes the conformal prediciton.
...
When we run this estimator, we need to specify one of three estimators: ```TLP```, ```SBMF```, or ```both```. We also need to supply a dictionary entry called ```addout``` which means the additional outcomes we care about, which in this case is ```"addout": list(df.columns[4:]```. When we run the estimator, we get:
