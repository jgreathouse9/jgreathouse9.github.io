...
SCMs are widely used in public policy and business analytics to measure the causal effect of laws or marketing interventions. However, many modern SCMs are *practically* hard to use due to technical debt.
...
- Handle multiple estimators seamlessly.
...
- A lightweight Python package with many machine learning based SCM estimators. Each class relies on a single grammar.
...
- **For beginners**:  
  - Simple, clear API  
  - One base syntax with many estimators  
...
- **For developers**:  
  ...
  - Lower technical debt means faster experimentation with different models.
...
- Pre-treatment period: 224 weeks (2011 Week 1, intervention began July 2015)
- Objective: Compare multiple SCM approaches to estimate the causal effect
- `mlsynth` allows this to be done seamlessly in Python in under 3 minutes and in under 30 lines of code
...
We were able to estimate the causal impact of Barcelona's hotel moratorium using 3 different approaches in under two minutes, not hours. 

Barring changes to the base dictionary call, `mlsynth`'s common base syntax allows us to switch between estimators without needing to learn a new syntax structure.
