...
## [Key Takeaways for Practitioners](https://arxiv.org/pdf/2203.06279)
...
- **Low-volatility series**  
  ...
  *Condition (informal):* the pre-treatment volatility should be small relative to the signal, e.g.
  $$
  \frac{\operatorname{sd}(y_{1t\le T_0})}{\overline{y}_{1t\le T_0}} \quad\text{is small.}
  $$
...
- **Small, relevant donor pool**  
  ...
  *Condition (selection):* restrict donor count $J$ relative to pre-period information, e.g.
  ...
  $$
  \min_{\mathcal{D}\subseteq\{1,\dots,J\}} \; \operatorname{dist}\big(y_{1t\le T_0},\, \{y_{jt\le T_0}: j\in\mathcal{D}\}\big).
  $$
...
- **Sparsity aids interpretability**  
  ...
  *Condition:* prefer weight vectors $\mathbf{w}$ with small support:
  $$
  \|\mathbf{w}\|_0 \text{ small} \quad\text{or enforce sparsity via the} $\ell_1$ norm: \|\mathbf{w}\|_1
  $$
...
- **Plot your treated unit versus the donors (seriously)**  
  ...
  *Diagnostic:* inspect $\{y_{jt}\}_{j\in \mathcal{J}_{0}}$ vs.\ $y_{1t}$ for $t\le T_0$.
