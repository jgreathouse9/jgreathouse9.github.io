...
The donor with the lowest MSE is selected as the initial candidate:

$$
 j^\ast = \arg\min_{j \in \mathcal{N}_0} \| \mathbf{y}_{1,\mathcal{T}_1} - \mathbf{Y}_{0,\mathcal{T}_1} \mathbf{w}^{(j)} \|_2^2.
$$

Next, we iteratively expand the donor pool by adding the control unit %j \in \mathcal{N}_0 \setminus \mathcal{S}$ that, when included, minimizes the MSE over the new subsimplex:
