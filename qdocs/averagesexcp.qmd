...
Now we substitute this result into the second term on the left hand side. That gives us this result:

$$
\sum_{i=1}^{n} x_i - n\mu = 0.
$$

Now we seek to isolate $\mu$. To do this, we first add $n\mu$ to the RHS

$$
\sum_{i=1}^{n} x_i = n\mu.
$$

Finally, dividing both sides by $n$ gives us this result:
...
## The Arithmetic Mean as a Projection

We can also understand the arithmetic mean entirely through geometry. Instead of viewing each observation $x_i$ as a separate number, collect the data into a vector
...
where $\mathbf{1} = (1,1,\dots,1)^\top$. Each such vector corresponds to choosing a single number and repeating it across all observations. The set of all such vectors

$$
C = \{ \mu \mathbf{1} : \mu \in \mathbb{R} \}
$$

forms a one-dimensional subspace of $\mathbb{R}^n$, spanned by $\mathbf{1}$. Geometrically, this is a line through the origin in the direction of $\mathbf{1}$. The problem of finding the arithmetic mean is equivalent to asking which point on this line is closest to the data vector $\mathbf{x}$ in Euclidean distance. Closeness is measured by squared distance, which gives exactly the same objective function as before:

$$
\|\mathbf{x} - \mu \mathbf{1}\|^2 = \sum_{i=1}^n (x_i - \mu)^2.
$$

One way to think about this intuitively is to imagine the vector of data points as a small cloud in $n$-dimensional space. The subspace $C$ is a line running through the origin, and the projection of the data vector onto this line is the point on the line that comes closest to the entire cloud. You can think of it as the line that best "fits" said cloud in the sense of minimizing Euclidean distance from each point to the line.

A basic fact from linear algebra is that the closest point in a subspace to a given vector is obtained by orthogonal projection. This means that at the optimal value $\mu$, the residual vector

$$
\mathbf{x} - \mu \mathbf{1}
$$

is perpendicular to the subspace $C$, which is spanned by $\mathbf{1}$. Orthogonality can be expressed as

$$
\mathbf{1}^\top (\mathbf{x} - \mu \mathbf{1}) = 0.
$$

Expanding this gives

$$
\mathbf{1}^\top \mathbf{x} - \mu \mathbf{1}^\top \mathbf{1} = 0.
$$

The two inner products are

$$
\mathbf{1}^\top \mathbf{x} = \sum_{i=1}^n x_i, \quad \mathbf{1}^\top \mathbf{1} = n,
$$
...
which, following the calculus argument above, gives

$$
\mu = \frac{1}{n} \sum_{i=1}^n x_i.
$$

Thus the arithmetic mean is the projection of the data vector onto the subspace of constant vectors. The derivative-based condition from the calculus approach is equivalent to this orthogonality condition, however it is expressed componentwise.


The above argument formalizes an intuition we already have in one dimension. If we observe the numbers 6 and 7, we rarely hesitate to say that the number closest to both, under squared distance, is 6.5. Moving slightly to the left of 6.5 increases the distance to 7 more than it decreases the distance to 6; moving slightly to the right does the opposite. At 6.5, these forces exactly balance. The derivative simply encodes this balance condition. For me anyways, the calculus derivation and the geometry really helped me understand this idea better.

Either way, the arithmetic mean may *feel* objective when we're comparing one time series to another (say, when comparing a data markets in a region to a national average), but that is only because the optimization problem that defines it has been hidden by decades of pedagogy. 
